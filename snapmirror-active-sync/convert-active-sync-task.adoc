...
---
= Converter um relacionamento SAN existente do ONTAP SnapMirror para SnapMirror active sync para relacionamento SAN
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Se você configurou a proteção SnapMirror, pode converter um relacionamento SAN para SnapMirror active sync para SAN. A partir do ONTAP 9.15.1, você pode converter o relacionamento para usar proteção ativo-ativo simétrica.

Para obter informações sobre como converter um relacionamento SVM-DR em um relacionamento de sincronização ativa SnapMirror para NAS, consulte link:protect-nas-task.html#convert-an-svm-dr-relationship-to-a-snapmirror-active-sync-relationship["Converter um relacionamento SVM DR em um relacionamento de sincronização ativa do SnapMirror"].
