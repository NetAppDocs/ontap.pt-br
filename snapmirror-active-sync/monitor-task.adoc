...
---
= Monitorar operações de sincronização ativas do ONTAP SnapMirror
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
Você pode monitorar o status e o progresso de uma operação de failover planejada usando o comando  `snapmirror failover show`. Por exemplo, em uma configuração SAN:
