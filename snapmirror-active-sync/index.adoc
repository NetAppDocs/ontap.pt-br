...
Em configurações ativo-passivo assimétricas, os dados no site secundário são encaminhados para um LUN, namespace ou unidade de armazenamento.
...
Prioridade primária e viés primário são suportados no SnapMirror active sync a partir do ONTAP 9.15.1. As cópias primárias são designadas no System Manager e exibidas com a API REST do ONTAP e a CLI do ONTAP.
...
* NVMe para cargas de trabalho VMware
 ...
 a| 
* Ativo-passivo assimétrico



NOTE: Ativo-passivo não é compatível com ASA r2 e NVMe. Para obter mais informações sobre a compatibilidade com NVMe, consulte link:../nvme/support-limitations.html["Configuração, suporte e limitações do NVMe"].
