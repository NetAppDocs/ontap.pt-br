...
---
= Executar um failover planejado de clusters ONTAP em um relacionamento de sincronização ativa do SnapMirror
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
Você pode querer realizar um failover planejado para testar a integridade da sua configuração de recuperação de desastres ou para realizar manutenção no cluster primário.
...
. Para monitorar o status do failover, use o  `snapmirror failover show` comando na CLI do ONTAP.
...
+
Exemplo de failover em uma configuração SAN:

+
destination::>snapmirror failover start -destination-path vs1_dst:/cg/cg_dst

+
Exemplo de failover em uma configuração NAS:

+
destino::>snapmirror failover start -destination-path vs_b:

. A partir do destino, monitore o progresso do failover:
+
