...
---
= Remover uma configuração de sincronização ativa do ONTAP SnapMirror
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
== Remover uma configuração SAN assimétrica

* Antes de excluir a relação SAN de sincronização ativa do SnapMirror, todos os LUNs no cluster de destino devem ser desvinculados.
...
. Na janela *Excluir configuração de replicação*, marque *Liberar os snapshots base do volume de origem.* e selecione *Excluir*.
...
. No cluster de origem, navegue até *Proteção > Replicação*.
. Localize o relacionamento que deseja remover, selecione image:icon_kabob.gif["Ícone de opções do menu"] e escolha *Excluir*.
. Na janela *Excluir configuração de replicação*, marque *Liberar os snapshots base da storage VM de origem* e selecione *Excluir*.
...
----
+
O SVM e seus volumes são removidos do cluster secundário.
