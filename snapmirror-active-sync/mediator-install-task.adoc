...
* ONTAP Mediator 1.12.1 ou posterior está instalado em um host com uma CPU de 2 núcleos.
...
IMPORTANT: Se você estiver usando SnapMirror active sync e ONTAP Mediator ou ONTAP Cloud Mediator com ONTAP 9.17.1, consulte a seção *Problemas e limitações conhecidos* link:https://library.netapp.com/ecm/ecm_download_file/ECMLP2492508["Notas de versão do ONTAP"^] para obter informações importantes sobre essas configurações.
...
Ao usar ONTAP Mediator para monitorar um SnapMirror relacionamento de sincronização ativo, ONTAP Mediator não pode ser inicializado no ONTAP sem um certificado autoassinado ou de uma autoridade certificadora (CA) válido. Você adiciona um certificado válido ao repositório de certificados para clusters emparelhados.
...
-----END CERTIFICATE-----
----
. Instale os certificados de CA raiz e intermediários no cluster primário ou secundário:
+
...
+
Digite o  `root.crt` quando solicitado, conforme mostrado no exemplo a seguir:
...
----
. Repita o passo anterior para instalar os certificados CA raiz e intermediários no cluster emparelhado.
. Veja os certificados de CA autoassinados instalados usando o nome do certificado gerado:
+
