...
Ao usar ONTAP Mediator para monitorar um SnapMirror relacionamento de sincronização ativo, ONTAP Mediator não pode ser inicializado no ONTAP sem um certificado autoassinado ou de uma autoridade certificadora (CA) válido. Você adiciona um certificado válido ao repositório de certificados para clusters emparelhados. Ao usar ONTAP Mediator para monitorar MetroCluster sistemas IP, o HTTPS não é usado após a configuração inicial; portanto, certificados não são necessários.
...
-----END CERTIFICATE-----
----
. Adicione o certificado CA do ONTAP Mediator a um cluster ONTAP. Quando solicitado, insira o certificado CA obtido do ONTAP Mediator. Repita as etapas em todos os clusters pares:
+
