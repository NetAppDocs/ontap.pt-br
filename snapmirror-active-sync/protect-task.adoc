...
---
= Proteja com ONTAP SnapMirror active sync para SAN
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Configurar a proteção assimétrica para configurações SAN usando SnapMirror active sync envolve selecionar LUNs no cluster de origem ONTAP e adicioná-las a um grupo de consistência.
...
* O cluster de origem e o cluster de destino não podem fazer parte de uma configuração MetroCluster.
...
Você pode estabelecer proteção simétrica para configurações SAN usando System Manager ou a ONTAP CLI. Em ambas as interfaces, existem etapas diferentes para xref:index.html#key-concepts[configurações uniformes e não uniformes].
...
. No site principal, link:../consistency-groups/configure-task.html#create-a-consistency-group-with-new-luns-or-volumes["crie um grupo de consistência usando novos LUNs"].
+
