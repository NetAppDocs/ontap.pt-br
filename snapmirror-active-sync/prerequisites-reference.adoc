---
sidebar: sidebar 
permalink: snapmirror-active-sync/prerequisites-reference.html 
keywords: SM-BC, SMBC, prerequisites, planning, software, hardware, licensing, protocols, network, snapmirror active sync 
summary: Existem vários pré-requisitos que você deve considerar como parte do Planejamento de uma implantação de solução de sincronização ativa do SnapMirror. 
---
= Pré-requisitos para sincronização ativa do ONTAP SnapMirror
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
:linkattrs: 
:imagesdir: ../media/


[role="lead"]
Ao Planejar sua implantação de sincronização ativa do SnapMirror, verifique se você atendeu aos vários requisitos de hardware, software e configuração do sistema.



[[hardware]]
== Hardware

.Suporte a SAN
A tabela a seguir descreve as configurações de cluster do NetApp compatíveis.

[cols="25,25,25,25"]
|===


| Tipo de cluster | Modelos suportados | Recursos suportados | Nós de cluster máximos suportados 


 a| 
AFF
 a| 
Série a, série C.
 a| 
Failover automatizado duplex (ativo-ativo assimétrico), failover automatizado (ativo-passivo assimétrico)
 a| 
* 2 (ONTAP 9.9.1 ou posterior)
* 4 (ONTAP 9.16.1 com configurações ativo-ativo simétricas)




 a| 
ASA
 a| 
Série a, série C.
 a| 
Failover Duplex automatizado (simétrico ativo-ativo), failover automatizado (assimétrico ativo-passivo)
 a| 
* 2 (ONTAP 9.9.1 ou posterior
* 4 (ONTAP 9.16.1 com configurações ativo-ativo simétricas)




 a| 
ASA r2
 a| 
Tudo
 a| 
Duplex de failover automatizado (simétrico ativo-ativo)
 a| 
* 2 (ONTAP 9.17.1 ou anterior)
* 4 (ONTAP 9.18.1 ou posterior)


|===
.Suporte a NAS
A tabela a seguir descreve as configurações de cluster do NetApp compatíveis.

[cols="25,25,25,25"]
|===
| Tipo de cluster | Modelos suportados | Recursos suportados | Nós de cluster máximos suportados 


 a| 
AFF
 a| 
Série a, série C.
 a| 
Ativo-passivo (failover automatizado)
 a| 
2 (ONTAP 9.19.1 ou posterior)



 a| 
AFX
 a| 
Tudo
 a| 
Ativo-passivo (failover automatizado)
 a| 
4 (ONTAP 9.19.1 ou posterior)

|===
.Replicação entre plataformas
A tabela abaixo descreve a capacidade de replicação entre tipos de cluster.

[cols="33,33,33"]
|===


| Tipo de cluster 1 | Tipo de cluster 2 | Replicação compatível? 


 a| 
AFF Série A
 a| 
Série C da AFF
 a| 
Sim



 a| 
AFF
 a| 
AFX
 a| 
Não



 a| 
ASA r2 Série A
 a| 
ASA r2 Série C
 a| 
Sim



 a| 
AFF
 a| 
ASA
 a| 
Não



 a| 
ASA
 a| 
ASA r2
 a| 
Não

|===


[[software]]
== Software

* ONTAP 9.9,1 ou posterior
* ONTAP Mediador 1,2 ou posterior
* Um servidor Linux ou máquina virtual para o ONTAP Mediator executando um dos seguintes:


[cols="30,70"]
|===


| ONTAP versão mediadora | Versões Linux suportadas 


 a| 
1.12.1
 a| 
* Red Hat Enterprise Linux
+
** Compatível: 10.1 e 9.7 ^1^
** Recomendado: 10.2, 10.0, 9.8, 9.6, 9.4 e 8.10


* Rocky Linux 10.2, 9.8 e 8.10
* Oracle Linux 10.1 e 9.7




 a| 
1,12
 a| 
* Red Hat Enterprise Linux
+
** Compatível: 10.1 e 9.7 ^1^
** Recomendado: 10.2, 10.0, 9.8, 9.6, 9.4 e 8.10


* Rocky Linux 10.2, 9.8 e 8.10
* Oracle Linux 10.1 e 9.7




 a| 
1,11
 a| 
* Red Hat Enterprise Linux
+
** Compatível: 9,5 ^1^
** Recomendado: 10.1, 10.0, 9.7, 9.6, 9.4 e 8.10


* Rocky Linux 10,1, 9.7 e 8,10
* Oracle Linux 10.0 e 9.6




 a| 
1,10
 a| 
* Red Hat Enterprise Linux
+
** Compatível: 9,5 ^1^
** Recomendado: 10.0, 9.6, 9.4 e 8,10


* Rocky Linux 10,0, 9.6 e 8,10




 a| 
1.9.1
 a| 
* Red Hat Enterprise Linux
+
** Compatível: 9.3, 9.1, 8.9, 8.7, 8.6, 8.5 e 8.4 ^1^
** Recomendado: 9,5, 9,4, 9,2, 9,0, 8,10 e 8,8


* Rocky Linux 9.5 e 8.10




 a| 
1,9
 a| 
* Red Hat Enterprise Linux
+
** Compatível: 9.3, 9.1, 8.9, 8.7, 8.6, 8.5 e 8.4 ^1^
** Recomendado: 9,5, 9,4, 9,2, 9,0, 8,10 e 8,8


* Rocky Linux 9.5 e 8.10




 a| 
1,8
 a| 
* Red Hat Enterprise Linux:
+
** Compatível: 8.7, 8.6, 8.5 e 8.4 ^1^
** Recomendado: 9.4, 9.3, 9.2, 9.1, 9.0, 8.10, 8.9 e 8.8


* Rocky Linux 9.4 e 8.10




 a| 
1,7
 a| 
* Red Hat Enterprise Linux:
+
** Compatível: 8.7, 8.6, 8.5 e 8.4 ^1^
** Recomendado: 9.3, 9.2, 9.1, 9.0, 8.9 e 8.8


* Rocky Linux 9.3 e 8.9




 a| 
1,6
 a| 
* Red Hat Enterprise Linux:
+
** Compatível: 8.7, 8.6, 8.5 e 8.4 ^1^
** Recomendado: 9.2, 9.1, 9.0 e 8,8


* Rocky Linux 9.2 e 8.8




 a| 
1,5
 a| 
* Red Hat Enterprise Linux: 8.5, 8.4, 8.3, 8.2, 8.1, 8.0, 7.9, 7.8, 7.7 e 7.6
* CentOS: 7.9, 7.8, 7.7 e 7.6




 a| 
1,4
 a| 
* Red Hat Enterprise Linux: 8.5, 8.4, 8.3, 8.2, 8.1, 8.0, 7.9, 7.8, 7.7 e 7.6
* CentOS: 7.9, 7.8, 7.7 e 7.6




 a| 
1,3
 a| 
* Red Hat Enterprise Linux: 8.3, 8.2, 8.1, 8.0, 7.9, 7.8, 7.7 e 7.6
* CentOS: 7.9, 7.8, 7.7 e 7.6




 a| 
1,2
 a| 
* Red Hat Enterprise Linux: 8.1, 8.0, 7.9, 7.8, 7.7 e 7.6
* CentOS: 7.9, 7.8, 7.7 e 7.6


|===
. Compatível significa que o Red Hat não oferece mais suporte a essas versões do RHEL, mas o ONTAP Mediator ainda pode ser instalado nelas.




[[licensing]]
== Licenciamento

As seguintes licenças do SnapMirror estão disponíveis como parte do conjunto de licenças ONTAP One e devem ser aplicadas em ambos os clusters:

* SnapMirror síncrono
* SnapMirror
+

NOTE: Se seus sistemas de armazenamento ONTAP foram adquiridos antes de junho de 2019, consulte link:https://mysupport.netapp.com/site/systems/master-license-keys["Chaves de licença ONTAP"^] para obter a licença síncrona SnapMirror necessária.

* Para o vSphere Metro Storage Cluster (vMSC), é necessária uma licença do VMware vSphere.




[[networking-environment]]
== Ambiente de rede

* O tempo de ida e volta (RTT) de latência entre clusters deve ser inferior a 10 milissegundos.
* A partir do ONTAP 9.14.1, link:https://kb.netapp.com/onprem/ontap/da/SAN/What_are_SCSI_Reservations_and_SCSI_Persistent_Reservations["Reservas persistentes SCSI-3"^] há suporte para SnapMirror active sync.




[[supported-protocols]]
== Protocolos compatíveis

SnapMirror active sync oferece suporte aos seguintes protocolos SAN.

* Os protocolos FC e iSCSI são suportados a partir do ONTAP 9.9.1.
* O protocolo NVMe é compatível com cargas de trabalho VMware a partir do ONTAP 9.17.1.
+
A sincronização ativa do SnapMirror não oferece suporte ao seguinte com o protocolo NVMe:

+
** Configurações ativas/ativas simétricas de 4 nós
** Configurações ativo-passivo assimétricas
** Mudanças no tamanho do grupo de consistência
+
Não é possível expandir ou reduzir um grupo de consistência sem interrupção ao usar o protocolo NVMe com SnapMirror active sync. As operações de expansão e redução de grupos de consistência são disruptivas ao usar o protocolo NVMe com SnapMirror active sync.

** Coexistência de LUNs e namespaces no mesmo grupo de consistência.




A partir do ONTAP 9.19.1, o SnapMirror active sync oferece suporte aos seguintes protocolos NAS.

* NFSv3 e versões posteriores
+
O failover sem interrupção é compatível com NFSv3 e versões posteriores.

* SMB
+
** O failover sem interrupção é suportado para compartilhamentos SMB 3 CA.
** O failover disruptivo (mas com RPO zero mantido) é suportado para compartilhamentos SMB2 e SMB3 não CA.
** SMB1 não é suportado.




Os clusters NAS exigem o mesmo endereço IP e configuração de protocolo no cluster primário e no cluster secundário para permitir o failover sem interrupções. Como as SVMs podem ser configuradas com o mesmo endereço IP e protocolos, SnapMirror active sync oferece suporte a configurações NAS no nível da SVM; no entanto, a SVM secundária permanece inativa (não está em execução) porque as LIFs devem permanecer desativadas até que ocorra um failover para evitar colisão de IP com a SVM primária.



[[ipspace]]
== IPspace

O espaço IP padrão é exigido pela sincronização ativa do SnapMirror para relacionamentos entre pares de cluster. Espaços IP personalizados não são suportados.

Com o SnapMirror active sync para NAS, as SVMs primária e secundária podem estar em diferentes IPspaces.



[[ntfs-security-style]]
== Estilo de segurança NTFS

O estilo de segurança NTFS *não* é suportado em volumes do SnapMirror active sync para SAN. O estilo de segurança NTFS é suportado com o SnapMirror active sync para NAS.



[[ontap-mediator]]
== ONTAP Mediador

* O ONTAP Mediator deve ser provisionado externamente e anexado ao ONTAP para failover de aplicativo transparente.
* Para ser totalmente funcional e permitir failover automático não planejado, o Mediador ONTAP externo deve ser provisionado e configurado com clusters ONTAP.
* O ONTAP Mediator deve ser instalado em um terceiro domínio de falha, separado dos dois clusters ONTAP.
* Ao instalar o ONTAP Mediator, você deve substituir o certificado autoassinado por um certificado válido assinado por uma CA confiável.
* Para obter mais informações sobre o ONTAP Mediator, consulte link:../mediator/index.html["Preparar para instalar o ONTAP Mediator"] .




[[other-prerequisites]]
== Outros pré-requisitos

* Em versões anteriores ao ONTAP 9.15.1, os relacionamentos de sincronização ativa do SnapMirror não são suportados em volumes de destino de leitura/gravação (volumes convertidos de DP para leitura/gravação em um ativo-passivo assimétrico). Antes de usar um volume de leitura/gravação, você deve convertê-lo em um volume DP criando uma relação do SnapMirror em nível de volume (assíncrona ou síncrona) e, em seguida, excluindo a relação. Para obter detalhes, consulte link:convert-active-sync-task.html["Converta relações SnapMirror existentes para a sincronização ativa do SnapMirror"].
* VMs de armazenamento que usam a sincronização ativa do SnapMirror não podem ser associadas ao Active Directory como um computador cliente.




[[further-information]]
== Mais informações

* link:https://hwu.netapp.com/["Hardware Universe"^]
* link:../mediator/mediator-overview-concept.html["Visão geral do Mediador ONTAP"]

