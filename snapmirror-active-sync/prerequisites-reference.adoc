...
Série a, série C.
 a| 
Failover Duplex Automatizado (ativo/ativo simétrico), Failover Automatizado (ativo-passivo assimétrico)
 a| 
...
* Em versões anteriores ao ONTAP 9.15.1, os relacionamentos de sincronização ativa do SnapMirror não são suportados em volumes de destino de leitura/gravação (volumes convertidos para leitura/gravação a partir de DP em um ativo-passivo assimétrico). Antes de poder usar um volume de leitura/gravação, você deve convertê-lo em um volume DP criando uma relação do SnapMirror em nível de volume (assíncrona ou síncrona) e, em seguida, excluindo a relação. Para obter detalhes, consulte link:convert-active-sync-task.html["Converta relações SnapMirror existentes para a sincronização ativa do SnapMirror"].
