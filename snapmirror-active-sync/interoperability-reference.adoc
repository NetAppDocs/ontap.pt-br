...
A partir do ONTAP 9.15.1, o SnapMirror active sync oferece suporte à reconfiguração automática no ramo de fan-out após um evento de failover. Se o failover do site primário para o secundário for bem-sucedido, o site terciário é reconfigurado automaticamente para tratar o site secundário como a origem. O ramo de fan-out assíncrono pode ser um relacionamento de grupo de consistência em uma configuração SAN ou um relacionamento de volume independente. A reconfiguração funcionará em ambos os casos. A reconfiguração é acionada por um failover planejado ou não planejado. A reconfiguração também ocorre ao retornar ao site primário.
