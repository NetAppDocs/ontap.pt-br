...
Para obter informações adicionais sobre a compatibilidade com configurações NAS, consulte link:protect-nas-task.html["Proteja com ONTAP SnapMirror active sync para NAS"].
...
* AIX
+
** A partir do ONTAP 9.19.1, o Transparent Application Failover (TAF) é suportado com configurações ativo-ativo simétricas. Não é necessário PVR.
** A partir do ONTAP 9.11.1 até o ONTAP 9.18.1, o AIX é suportado apenas por PVR para failover automatizado com configurações simétricas ativo-ativo.
...
A partir do ONTAP 9.19.1, o Failover Transparente de Aplicativos para AIX é suportado com SnapMirror active sync em configurações simétricas ativo-ativo. Você pode usar System Manager ou a ONTAP CLI para criar um LUN AIX. Para garantir o failover transparente, ao usar a ONTAP CLI, você deve usar a opção access-mode local com o comando lun create ou lun modify. Por padrão, o modo de acesso é definido como global. Ao usar o System Manager, escolha AIX como host e local como modo de acesso. Para obter mais informações sobre como criar um LUN, consulte link:../san-admin/provision-storage.html["Provisionamento de storage SAN"]. Para obter informações adicionais sobre o suporte para AIX, consulte o artigo da Knowledge Base da NetApp: link:https://kb.netapp.com/on-prem/ontap/DP/SnapMirror/SnapMirror-KBs/SnapMirror_active_sync_support_for_AIX%3A_behavior_by_ONTAP_release_and_migration_guidance["SnapMirror active sync suporte para AIX: comportamento por release do ONTAP e orientações de migração"^].

Se você estiver usando ONTAP 9.11.1 até ONTAP 9.18.1, o AIX é compatível com SnapMirror active sync por meio de uma Solicitação de Variação de Política de Recursos (FPVR) de engenharia padrão, desde que as seguintes estipulações sejam compreendidas:
...
Um evento de failover automático não planejado (AUFO) no cluster primário isolado pode ser causado por uma falha dupla quando a conexão entre o cluster primário e o secundário é perdida e a conexão entre o cluster primário e o mediador também é perdida. Isso é considerado um evento raro, ao contrário de outros eventos AUFO.
...
A partir do ONTAP 9.19.1, o ARP/AI oferece suporte a volumes primários que participam de relacionamentos SAN de sincronização ativa do SnapMirror. Para obter mais informações, consulte link:../anti-ransomware/use-cases-restrictions-concept.html#interoperability-and-vm-considerations["Casos de uso e considerações da proteção autônoma contra ransomware"].
...
SnapMirror active sync para SAN é compatível xref:../data-protection/supported-deployment-config-concept.html[configurações de fan-out] com a  `MirrorAllSnapshots` política e, a partir do ONTAP 9.11.1, com a  `MirrorAndVault` política. Configurações de fan-out não são compatíveis com SnapMirror active sync com a  `XDPDefault` política.

A partir do ONTAP 9.15.1, o SnapMirror active sync oferece suporte à reconfiguração automática no ramo de distribuição (fan-out) após um evento de failover. Se o failover do site primário para o secundário for bem-sucedido, a reconfiguração do relacionamento de distribuição será iniciada durante a próxima atualização agendada do ramo de distribuição assíncrono. Ela não é acionada imediatamente após o failover do SnapMirror active sync. O ramo de distribuição assíncrono pode ser um relacionamento de grupo de consistência em uma configuração SAN ou um relacionamento de volume independente. A reconfiguração funcionará em ambos os casos. A reconfiguração é acionada por um failover planejado ou não planejado. A reconfiguração também ocorre no retorno ao site primário.
...
A partir do ONTAP 9.12.1, link:../data-protection/restore-part-file-snapshot-task.html["Restauração parcial de LUN"] é compatível com volumes de sincronização ativa do SnapMirror. Você pode restaurar dados de snapshots criados por aplicativos que foram replicados entre o volume de origem da sincronização ativa do SnapMirror e os volumes de destino (snapshot). A restauração parcial de LUN ou arquivo pode ser necessária se você precisar restaurar um banco de dados em um host que armazena vários bancos de dados no mesmo LUN. Para usar essa funcionalidade, você precisa saber o deslocamento inicial em bytes dos dados e a contagem de bytes.
...
Snapshots à prova de adulteração não são suportados com SnapMirror active sync.
