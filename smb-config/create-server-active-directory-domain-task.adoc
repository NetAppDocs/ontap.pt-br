...
Você precisa dos seguintes privilégios mínimos na unidade organizacional (OU) para criar o servidor SMB:
...
* Gravação validada em service principal names
* Ler msDS-SupportedEncryptedTypes
* Grave msDS-SupportedEncryptedTypes
...
* Você pode optar por adicionar o servidor SMB a uma Unidade Organizacional (UO) diferente usando a opção  `-ou`.
