...
[role="lead"]
Se você habilitou as regras de política de exportação SMB, a política de exportação padrão do volume raiz do SVM deve incluir uma regra para permitir que todos os clientes tenham acesso aberto por meio de SMB. Sem essa regra, todos os clientes SMB terão o acesso negado ao SVM e aos seus volumes. As regras de política de exportação estão desabilitadas por padrão.
...
link:../smb-admin/enable-disable-export-policies-task.html["Aprenda a ativar ou desativar as políticas de exportação do ONTAP para acesso SMB"].
