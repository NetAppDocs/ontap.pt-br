...
. Modifique o servidor de chave primária para adicionar servidores de chave secundária. O `-secondary-key-servers` O parâmetro aceita uma lista de até três servidores-chave, separados por vírgulas. Não inclua um número de porta para servidores de chave secundários.
...
--
. Modifique o servidor de chave primária para adicionar servidores de chave secundária. O `-secondary-key-servers` O parâmetro aceita uma lista de até três servidores-chave, separados por vírgulas. Não inclua um número de porta para servidores de chave secundários.
`security key-manager external modify-server -vserver _svm_name_ -key-servers _primary_key_server_ -secondary-key-servers _list_of_key_servers_` Para obter mais informações sobre servidores de chaves secundários, consulte <<mod-secondary>>.
...
Os servidores de chave secundários são gerenciados com o `-secondary-key-servers` parâmetro do `security key-manager external modify-server` comando. O `-secondary-key-servers` O parâmetro aceita uma lista separada por vírgulas. A ordem especificada dos servidores de chaves secundárias na lista determina a sequência de acesso para esses servidores. A ordem de acesso pode ser modificada executando o comando. `security key-manager external modify-server` com os servidores de chave secundários inseridos em uma sequência diferente. Não inclua um número de porta para servidores de chave secundários.
