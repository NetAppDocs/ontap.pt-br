...
* O  `scan-queue-timeout`valor deve ser menor que o  `scanner-pool request-timeout`valor. Se for definido com um valor maior, o acesso aos arquivos pode ser atrasado e eventualmente expirar. Para evitar isso, configure o  `scan-queue-timeout`para 5 segundos a menos que o  `scanner-pool request-timeout`valor. Consulte a documentação do fornecedor do mecanismo de varredura para obter instruções sobre como alterar as  `scan-queue-timeout`configurações. O  `scanner-pool timeout`pode ser alterado usando o seguinte comando no modo avançado e fornecendo o valor apropriado para o parâmetro  `request-timeout`:
