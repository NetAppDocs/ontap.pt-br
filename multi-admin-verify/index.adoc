...
---
= Visão geral da verificação de vários administradores
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. Antes que possa ser executado, pelo menos um link:manage-requests-task.html["O administrador do MAV deve aprovar."]
...
*Este comando só está disponível com CLI e não está disponível para o System Manager.
...
NOTE: Um solicitante não pode aprovar suas próprias solicitações, mesmo que estejam no grupo de aprovação. Eles podem receber notificações por e-mail. Os solicitantes que não estão em grupos de aprovação (ou seja, que não são administradores MAV) não recebem notificações por e-mail.
