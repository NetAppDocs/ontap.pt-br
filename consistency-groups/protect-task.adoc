...
---
= Proteja grupos de consistência ONTAP
:hardbreaks:
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. Marque a caixa ao lado de *Agendar Snapshots (local)*.
...
* Ao invocar a criação de snapshot, você pode definir um valor de tempo limite opcional entre 5 e 120 segundos. Se nenhum valor de tempo limite for fornecido, a operação expira no tempo limite padrão de 7 segundos. Na ONTAP API REST, defina o valor de tempo limite com o  `action_timeout` parâmetro. Na ONTAP CLI, use a  `-timeout` flag.


.Passos
Você pode concluir um snapshot em duas fases com a API REST do ONTAP ou, a partir do ONTAP 9.14.1, com a CLI do ONTAP. Essa operação não é compatível com o System Manager.


NOTE: Se você invocar a criação de snapshot com a ONTAP REST API, deverá confirmar o snapshot com a ONTAP REST API. Se você invocar a criação de snapshot com a ONTAP CLI, deverá confirmar o snapshot com a ONTAP CLI. A combinação de métodos não é suportada.
