...
.. Defina uma regra para a política de espelhamento e cofre que determina quais cópias snapshot serão transferidas durante as operações de inicialização e atualização: `snapmirror policy add-rule -vserver svm_name -policy policy_for_rule - snapmirror-label snapmirror-label -keep retention_count -schedule schedule`
