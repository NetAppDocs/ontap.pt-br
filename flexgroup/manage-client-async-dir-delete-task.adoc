...
Os direitos são concedidos por volume.

Você pode mover apenas diretórios.  Você não pode mover arquivos para o diretório .ontaptrashbin.

link:fast-directory-delete-asynchronous-task.html#delete-directories-asynchronously["Saiba como usar ONTAP para excluir diretórios de forma assíncrona dos volumes FlexVol e FlexGroup"].
...
.Passos
. A partir da CLI do cluster, entre no modo de privilégios avançados: `-privilege advanced`
