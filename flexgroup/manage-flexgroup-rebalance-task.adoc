...
+
É recomendável que você _não_ use o rebalanceamento automático do FlexGroup após uma conversão de FlexVol para FlexGroup .  Em vez disso, você pode redistribuir os arquivos existentes usando o `volume rebalance file-move start` comando, disponível no ONTAP 9.10.1 e posteriores.  Esta operação não é disruptiva por padrão(`-is-disruptive false` ).  Se alguns arquivos ocupados não puderem ser movidos, você pode executar o comando novamente no modo disruptivo(`-is-disruptive true` ) durante uma janela de manutenção planejada. Saiba mais sobre  `volume rebalance file-move start` no link:https://docs.netapp.com/us-en/ontap-cli/volume-rebalance-file-move-start.html["Referência do comando ONTAP"^] .
