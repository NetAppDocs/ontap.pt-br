...
== Funcionalidades suportadas a partir do ONTAP 9.18.1

* xref:../performance-admin/adaptive-qos-policy-groups-task.html[Políticas de QoS aninhadas]São suportados para os seguintes pares de objetos:
+
** Volumes SVM e FlexGroup contidos pelo SVM
** Volumes FlexGroup e qtrees dentro dos volumes
...
* xref:../anti-ransomware/index.html[Proteção autônoma contra ransomware] para os volumes FlexGroup, incluindo a seguinte funcionalidade suportada:
+
...
+
Para mais informações, consulte olink:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Storage_Software/ONTAP_OS/How_To_Convert_a_Transitioned_FlexVol_to_FlexGroup["Base de conhecimento da NetApp : Como converter um FlexVol em transição para FlexGroup"^] .
