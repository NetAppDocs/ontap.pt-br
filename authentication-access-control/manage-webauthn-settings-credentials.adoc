...
== Restringir o conjunto de domínios de Relying Party (RP) permitidos

A partir do ONTAP 9.19.1, os administradores devem usar o parâmetro -rp-domains no comando de configurações globais WebAuthn. Isso permite que os administradores restrinjam explicitamente o conjunto de domínios de Relying Party (RP) permitidos durante a autenticação multifator WebAuthn (FIDO2) no System Manager.

Os usuários só poderão acessar MFA a partir dos domínios RP permitidos.


NOTE: Se você estiver atualizando para ONTAP 9.19.1 a partir do ONTAP 9.16.1 até o 9.18.1, consulte link:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-Issues/CONTAP-597688["CONTAP-597688"] para mais informações.

[role="tabbed-block"]
====
.System Manager
--
. Selecione *Cluster > Configurações > autenticação multifator (MFA)*.
. Adicione, exclua ou modifique os domínios RP permitidos.


--
.CLI
--
. Modifique os domínios rp permitidos. Defina -rp-domains para o FQDN completo do cluster:
+
[source, console]
----
...
----
+
Saiba mais sobre  `security webauthn modify` na link:https://docs.netapp.com/us-en/ontap-cli/security-webauthn-modify.html#parameters["Referência do comando ONTAP"^].
