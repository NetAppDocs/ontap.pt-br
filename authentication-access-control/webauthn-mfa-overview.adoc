...
[NOTE]
====
A partir do ONTAP 9.19.1, os administradores devem usar o parâmetro  `-rp-domains` no comando de configurações globais do WebAuthn. Isso permite que os administradores restrinjam explicitamente o conjunto de domínios de Relying Party (RP) permitidos durante a autenticação multifator WebAuthn (FIDO2) no System Manager. Se você estiver atualizando para ONTAP 9.19.1 a partir do ONTAP 9.16.1 até o 9.18.1, consulte link:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-Issues/CONTAP-597688["CONTAP-597688"] para obter mais informações.
