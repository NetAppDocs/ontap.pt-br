...
NOTE: Cada porta RDMA pode ter um máximo de 127 LIFs. Ao adicionar mais LIFs, ou quando uma LIF falha e é transferida para uma porta que excede esse limite, as LIFs recém-adicionadas ou a LIF que falhou não podem atender ao tráfego RDMA.
...
. Selecione a máquina virtual de armazenamento e o nó principal. Atribua um *Nome*, um *endereço IP* e uma *Máscara de sub-rede*.
...
** O `-rdma-protocols` parâmetro aceita uma lista, que por padrão está vazia. Quando `roce` é adicionado como valor, o LIF só pode ser configurado em portas que suportam o offload RoCE, afetando tanto a migração quanto o failover do LIF.
