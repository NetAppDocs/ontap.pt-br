...
[role="lead"]
NetApp recomenda que você use o System Manager para criar novos clusters. System Manager fornece um fluxo de trabalho simples e fácil para a configuração do cluster. Só é necessário usar a interface de linha de comando do ONTAP (CLI) se você estiver executando ONTAP 9.7 ou anterior em uma configuração MetroCluster ou se precisar configurar um cluster somente IPv6 em sistemas compatíveis.

.Sobre esta tarefa
Este procedimento aplica-se a link:https://docs.netapp.com/us-en/ontap-systems-family/index.html["AFF, ASA, FAS"^] e link:https://docs.netapp.com/us-en/ontap-afx/index.html["Sistemas AFX"^]. Se você possui um sistema ASA r2 (ASA A1K, ASA A90, ASA A70, ASA A50, ASA A30, ASA A20 ou ASA C30), siga link:https://docs.netapp.com/us-en/asa-r2/install-setup/initialize-ontap-cluster.html["estes passos"^] para usar o System Manager para configurar um cluster ONTAP. Os sistemas ASA r2 oferecem uma experiência ONTAP simplificada, específica para clientes apenas SAN.

A atribuição de endereços IPv6 a LIFs de gerenciamento durante a configuração do cluster é compatível com os seguintes sistemas e versões do ONTAP.

[cols="2"]
|===
| Versão de ONTAP | Sistemas suportados 
...
Sua versão do BMC deve ser 19.3 ou posterior para suportar a atribuição de endereços IPv6 durante a configuração do cluster.



| ONTAP 9.13,1 e posterior  a| 
* AFF A800
* FAS8700


|===
Você deve realizar a configuração do cluster com a CLI do ONTAP para atribuir endereços IPv6 nos sistemas compatíveis. Se a atribuição de endereços IPv6 durante a configuração do cluster não for compatível com seu sistema e versão do ONTAP, você deve usar o System Manager para criar seu cluster usando endereços IPv4 e, em seguida, link:convert-ipv4-to-ipv6-task.html["Converter para IPv6"] atribuir os endereços após concluir a configuração do cluster.


NOTE: O Gerenciador de sistema não suporta implantações que exigem rede IPv6G no ONTAP 9,6 e versões anteriores.

.Antes de começar
* Você deve ter instalado, conectado os cabos e ligado o seu novo sistema de storage de acordo com as instruções de instalação e configuração do seu modelo.
+
Veja o link:https://docs.netapp.com/us-en/ontap-systems/index.html["Documentação dos sistemas de hardware ONTAP"^].

* Se os switches de cluster fizerem parte da instalação do sistema, revise as considerações de cabeamento e configuração do modelo do switch de cluster para as portas de cluster em seus novos sistemas de storage. Consulte link:https://docs.netapp.com/us-en/ontap-systems-switches/index.html["Documentação de switches para sistemas de hardware ONTAP"^].
+
