...
Quando você adiciona um novo disco a um par de HA, por exemplo, ao substituir um disco com falha, responder a uma mensagem de "poucos discos sobressalentes" ou adicionar capacidade, a política de atribuição automática padrão atribui a propriedade do disco a um nó como um disco sobressalente.
