...
summary: 'Você pode atribuir políticas de retenção de cache a volumes nas camadas locais (agregados) do Flash Pool para determinar por quanto tempo os dados de volume permanecem no cache do Flash Pool. No entanto, em alguns casos, alterar a política de retenção de cache pode não afetar o tempo que os dados do volume permanecem no cache.' 
---
