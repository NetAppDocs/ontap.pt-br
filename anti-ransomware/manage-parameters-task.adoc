...
NOTE: Para reduzir o número elevado de alertas falsos positivos, vá em *Storage > Volumes > Security > Configure workload characteristics* e desative *Monitor new file types*. Essa configuração está desativada por padrão no ONTAP 9.14.1 P7, 9.15.1 P1, 9.16.1 e versões posteriores. A partir do ONTAP 9.18.1 P2, o System Manager não exibe mais as características da carga de trabalho ARP, incluindo as estatísticas de baseline e de pico. As estatísticas de pico continuam disponíveis usando as opções da CLI do ONTAP e da API REST do ONTAP para análise avançada.
