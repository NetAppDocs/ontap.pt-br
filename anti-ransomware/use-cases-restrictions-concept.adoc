...
* A análise ARP/AI é executada no volume ativo de leitura e gravação em SnapMirror síncrono e SnapMirror active sync em relacionamentos SAN. O ARP cria snapshots no volume primário e realiza detecção de ransomware baseada em aprendizado de máquina.
* Os snapshots ARP não são replicados para o volume secundário em relacionamentos SnapMirror síncronos ou de sincronização ativa SnapMirror. Você precisa link:enable-task.html["ativar ARP novamente"] no volume de recuperação após o failover.
