...
* A partir do ONTAP 9.10.1 até 9.15.1, cargas de trabalho NAS em volumes com mais de 4.000 extensões de arquivo exclusivas.
+
O modelo ARP original consegue analisar até 4.000 extensões de arquivo por volume. Se o número de extensões de arquivo exclusivas em um volume exceder esse limite, o ARP não consegue detectar atividades de ransomware de forma eficiente e a carga de trabalho é considerada inadequada para o ARP. Esse limite não se aplica ao ARP/AI a partir do ONTAP 9.16.1.
...
|===
^1^ ONTAP 9.16.1 e 9.17.1 não oferecem suporte a ARP/AI para FlexGroup volumes; a partir do ONTAP 9.18.1, FlexGroup volumes usam o modelo ARP/AI. Para obter informações sobre o comportamento ao atualizar para essas versões, consulte <<arp-upgrade-considerations,Considerações específicas sobre a atualização da versão ARP>>.

^2^ Consulte <<arp-upgrade-considerations,Considerações específicas sobre a atualização da versão ARP>> para obter informações sobre o comportamento de capacitação padrão do ARP após a atualização para o ONTAP 9.18.1.

^3^ A partir do ONTAP 9.19.1, o ARP/AI oferece suporte a volumes primários em SnapMirror relacionamentos síncronos para NAS e SAN em sistemas FAS, AFF série A, AFF série C e AFX, e a volumes primários em SnapMirror relacionamentos SAN de sincronização ativa em sistemas AFF série A, AFF série C e ASA r2. Para obter mais informações, consulte <<SnapMirror>>.
...
* O estado de configuração ARP e os snapshots ARP não são replicados para o volume secundário como parte da SnapMirror síncrona ou SnapMirror active sync no ONTAP 9.19.1.



NOTE: Como cada operação de gravação é confirmada em ambos os volumes em uma relação de espelhamento síncrono, habilitar o ARP apenas no volume primário é suficiente para detectar as anomalias. Os metadados internos gerados pela análise de ARP no volume primário também são replicados para o volume secundário. Por isso, se o ARP for habilitado no volume secundário após uma falha, os metadados necessários já existirão nesse volume secundário.

Se um volume de origem SnapMirror _assíncrono_ tiver o ARP habilitado, o volume de destino SnapMirror adquire automaticamente o estado de configuração do ARP (como  `dry-run` ou  `enabled`), os dados de treinamento do ARP e o snapshot criado pelo ARP do volume de origem. Nenhuma capacitação é necessária.
...
Nas versões ONTAP 9.10.1 e 9.11.1, o SnapMirror não transfere o estado de configuração ARP, os dados de treinamento e os snapshots dos volumes de origem para os volumes de destino. Quando o volume de destino do SnapMirror é convertido para leitura e gravação, o ARP no volume de destino deve ser explicitamente habilitado no modo de aprendizado após a conversão.
...
== Considerações específicas sobre a atualização da versão ARP

Ao atualizar clusters ONTAP que já utilizam ARP, você deve estar ciente das alterações de comportamento específicas do ARP resultantes da atualização, além de link:../upgrade/special-considerations.html["Considerações especiais do ONTAP que você deve resolver"].



[[arp-default-enablement-after-upgrading-to-ontap-9-18-1]]
=== Capacitação padrão de ARP após atualização para ONTAP 9.18.1

A partir do ONTAP 9.18.1, o comportamento de capacitação padrão do ARP/AI está disponível para sistemas AFF série A e AFF série C, ASA e ASA r2. Após uma atualização, o ARP/AI é ativado automaticamente em todos os novos volumes após um período de carência de 12 horas. Você deve link:index.html#manual-default-enablement["habilitar ARP manualmente"] em volumes existentes.



[[snapmirror-relationships-after-upgrading-to-ontap-9-19-1]]
=== SnapMirror relationships após a atualização para ONTAP 9.19.1

Se você habilitar o ARP em volumes que fazem parte de SnapMirror síncronos ou de SnapMirror sincronização ativa após a atualização, o relacionamento entrará brevemente em um estado de dessincronização. Essa condição é temporária e o relacionamento deverá ressincronizar automaticamente sem intervenção.



[[flexgroup-volumes-after-upgrade]]
=== FlexGroup volumes após atualização

ONTAP 9.16.1 e 9.17.1 não oferecem suporte a ARP/AI para volumes FlexGroup. Após uma atualização para essas versões, volumes FlexGroup habilitados para ARP continuam a operar com o mesmo modelo ARP usado antes do ARP/AI. A partir do ONTAP 9.18.1, volumes FlexGroup usam o modelo ARP/AI.
