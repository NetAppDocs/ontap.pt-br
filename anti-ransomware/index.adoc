...
Para suporte a protocolos, adequação à carga de trabalho e requisitos específicos da versão, consulte link:use-cases-restrictions-concept.html["Casos de uso e considerações do ARP"].
...
A partir do ONTAP 9.18.1, o ARP é habilitado automaticamente por padrão em todos os novos volumes para AFF série A e AFF série C, ASA e ASA r2. Essa capacitação automática do ARP por padrão não se aplica a link:use-cases-restrictions-concept.html#unsupported-configurations["volumes ou configurações não suportadas"]. Você deve <<manual-default-enablement,habilitar ARP manualmente>> em volumes existentes. Para obter detalhes sobre como a capacitação padrão entra em vigor durante a atualização, consulte link:use-cases-restrictions-concept.html#arp-upgrade-considerations["Considerações específicas sobre a atualização da versão ARP"].


NOTE: A partir do ONTAP 9.19.1, a capacitação padrão de ARP se aplica a clusters que incluem SnapMirror síncrono e SnapMirror active sync em relacionamentos SAN. No ONTAP 9.18.1, a capacitação padrão foi suprimida para essas configurações. Para relacionamentos SnapMirror síncronos entre clusters, a capacitação padrão é ativada assim que ambos os clusters de origem e destino são atualizados para o ONTAP 9.19.1. O ARP não é ativado por padrão em sistemas AFX, mesmo com suporte síncrono do SnapMirror.
...
Avaliação
 a| 
Um período de avaliação de duas a quatro semanas é realizado para determinar o comportamento de criptografia de referência. Detecções e alertas podem ocorrer enquanto os limites de referência estão sendo estabelecidos. Você pode determinar se o período de avaliação foi concluído executando o comando  `security anti-ransomware volume show` e verificando  `Block device detection status`.

link:respond-san-entropy-eval-period.html["Saiba mais sobre o período de avaliação do ONTAP ARP/AI para cargas de trabalho em dispositivos de bloco"].
 a| 
...
.Diferenças no snapshot ARP por versão
* *Modificar configurações de retenção*: a partir do ONTAP 9.11.1, você pode link:modify-automatic-snapshot-options-task.html["modificar as configurações de retenção de snapshots"].
...
* *SnapMirror sincronização síncrona e ativa*: A partir do ONTAP 9.19.1, os snapshots ARP criados para volumes que participam de relacionamentos de SnapMirror sincronização síncrona ou SnapMirror sincronização ativa seguem estas regras adicionais:
+
** Os snapshots ARP são criados e mantidos no volume primário.
** Os snapshots ARP não são replicados para o secundário como parte da SnapMirror síncrona ou SnapMirror active sync. Você precisa link:enable-task.html["ativar ARP novamente"] no volume de recuperação após o failover.





NOTE: A restauração em nível de volume (`volume snapshot restore` em SnapMirror síncrono ou SnapMirror active sync em volumes SAN pode exigir que você suspenda temporariamente ou interrompa a relação SnapMirror síncrona ou SnapMirror active sync. Em muitos casos, você pode evitar interromper SnapMirror síncrono ou SnapMirror active sync usando FlexClone ou operações de restauração em nível de arquivo a partir de ARP ou outros snapshots no volume primário.

link:use-cases-restrictions-concept.html#snapmirror["Saiba mais sobre o suporte para SnapMirror síncrono e SnapMirror active sync"].
