...
.Sobre esta tarefa
Durante uma pausa ARP, o ONTAP não registra eventos ou ações para novas gravações; no entanto, a análise continua em segundo plano para registros anteriores.
...
. Se você estiver usando o MAV (disponível com ARP a partir do ONTAP 9.13.1) para proteger as configurações de ARP, a operação de pausa solicitará que você obtenha a aprovação de um ou mais administradores adicionais.  É necessário obter a aprovação de todos os administradores associados ao grupo de aprovação do MAV, caso contrário, a operação falhará.
+
