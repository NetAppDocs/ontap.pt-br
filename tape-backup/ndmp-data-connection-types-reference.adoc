...
summary: No modo NDMP com escopo de máquina virtual de armazenamento, os tipos de conexão de dados NDMP suportados dependem do tipo de LIF da conexão de controle NDMP e do status da extensão CAB. Esse tipo de conexão de dados NDMP indica se você pode executar uma operação de backup ou restauração NDMP local ou de três vias. 
---
