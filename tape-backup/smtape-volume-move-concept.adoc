...
[role="lead"]
As operações de backup do SMTape e as operações de movimentação de volume podem ser executadas em paralelo até que o sistema de storage tente a fase final de transição. Durante a fase final de transição, novas operações de backup do SMTape não podem ser executadas no volume que está sendo movido. No entanto, as operações em andamento continuam sendo executadas até a conclusão.
