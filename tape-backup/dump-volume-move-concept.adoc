...
[role="lead"]
As operações de backup em fita e restauração e a movimentação de volumes podem ser executadas em paralelo até que a fase final de migração seja tentada pelo sistema de storage. Durante a fase final de migração, novas operações de backup em fita e restauração não são permitidas no volume que está sendo movido. No entanto, as operações em andamento continuam sendo executadas até a conclusão.
