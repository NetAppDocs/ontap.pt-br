...
summary: Em um data center, você pode usar protocolos de descoberta de vizinhos para visualizar a conectividade de rede entre um par de sistemas físicos ou virtuais e suas interfaces de rede. ONTAP suporta dois protocolos de descoberta de vizinhos, Cisco Discovery Protocol e Link Layer Discovery Protocol. 
---
...
O CDP também deve ser ativado em todos os switches e roteadores Ethernet antes que as informações sobre dispositivos vizinhos possam ser exibidas.
...
NOTE: Às vezes, quando as LIFs são alteradas no nó, as informações do CDP não são atualizadas no dispositivo receptor (por exemplo, um switch Ethernet). Se você se deparar com esse problema, você deve configurar a interface de rede do nó para o status inativo e depois para o status ativo.
...
Vários switches de fornecedores não oferecem suporte ao CDP. Veja olink:https://kb.netapp.com/onprem/ontap/da/NAS/ONTAP_device_discovery_shows_nodes_instead_of_the_switch["Base de conhecimento da NetApp : a descoberta de dispositivos ONTAP mostra nós em vez do switch"^] para mais detalhes.
