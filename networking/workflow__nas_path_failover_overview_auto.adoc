...
summary: Configuração de failover de caminho NAS para ONTAP 9.8 e versões posteriores. 
---
= Fluxo de trabalho de failover de caminho NAS para o ONTAP
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
NOTE: Uma LIF SAN não migra (a menos que você a mova manualmente após a falha do link). Em vez disso, a tecnologia de multipathing no host redireciona o tráfego para uma LIF diferente. Para mais informações, consulte link:../san-admin/index.html["Administração da SAN"].
