...
Usando SnapLock, você grava arquivos e snapshots no armazenamento WORM e define períodos de retenção para os dados protegidos por WORM. O armazenamento WORM do SnapLock utiliza a tecnologia de snapshots da NetApp e pode aproveitar a replicação do SnapMirror e os backups do SnapVault como tecnologia base para fornecer proteção de recuperação de backup para os dados. Saiba mais sobre o armazenamento WORM: link:https://www.netapp.com/pdf.html?item=/media/6158-tr4526.pdf["Armazenamento WORM em conformidade com NetApp SnapLock - TR-4526"^].
