...
summary: 'Você usa a opção de volume  `-snaplock-type` para especificar um tipo de volume de Compliance ou Enterprise SnapLock. Para versões anteriores ao ONTAP 9.10.1, é necessário criar um agregado SnapLock separado. A partir do ONTAP 9.10.1, volumes SnapLock e não SnapLock podem existir no mesmo agregado. Portanto, não é mais necessário criar um agregado SnapLock separado se você estiver usando ONTAP 9.10.1.' 
---
= Criar um agregado ONTAP SnapLock
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
