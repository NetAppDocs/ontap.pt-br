...
O ONTAP impede que você reverta um cluster do ONTAP 9.10.1 para uma versão anterior do ONTAP quando há arquivos com um período de retenção posterior a "19 de janeiro de 2071 8:44:07 AM".
...
IMPORTANT: Em versões anteriores ao ONTAP 9.10.1, se você não definir explicitamente o tempo de retenção antes de confirmar um arquivo no modo de conformidade para o estado WORM e não modificar os valores padrão, o arquivo será retido por 30 anos. Essa alteração _não pode_ ser desfeita. Da mesma forma, no ONTAP 9.10.1 e versões posteriores, se você não definir explicitamente o tempo de retenção antes de confirmar um arquivo do modo Enterprise para o estado WORM e não modificar os valores padrão, o arquivo será retido por 0 anos ou, efetivamente, não será retido de forma alguma.
...
====
Para uso avançado, consulte link:https://www.netapp.com/pdf.html?item=/media/6158-tr4526.pdf["Storage WORM em conformidade com NetApp SnapLock"^].
