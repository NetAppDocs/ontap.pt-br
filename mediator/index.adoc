...
* Você pode usar qualquer versão do ONTAP Mediator para monitorar uma configuração de IP do MetroCluster.
...
Analise estes pontos antes de atualizar ou instalar ONTAP Mediator.
...
.Requisitos de rede
Certifique-se de que as seguintes portas estejam disponíveis e não estejam sendo usadas para o ONTAP Mediator:
...
|===
Para clientes SMBC, ONTAP não exige que a porta 3260 esteja habilitada ou conectada.

* Se estiver usando um firewall de terceiros, consulte link:https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/concept_mediator_requirements.html#firewall-requirements-for-ontap-mediator["Requisitos de firewall para ONTAP Mediator"^].
...
=== Considerações sobre atualização do sistema operacional e compatibilidade do kernel
...
* Não atualize o kernel para uma versão do sistema operacional superior à suportada para sua versão do ONTAP Mediator, pois o módulo SCST testado provavelmente não funcionará.
