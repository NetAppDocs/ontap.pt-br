...
[role="lead"]
A NetApp recomenda verificar a assinatura do código do Mediador ONTAP antes de instalar o pacote de instalação do Mediador ONTAP; no entanto, esse processo é opcional.
...
[NOTE]
====
* A partir de 15 de junho de 2025, você não poderá instalar ou atualizar para o ONTAP Mediator 1.8 e 1.9 porque os certificados de verificação de assinatura de código expiraram. Em vez disso, instale ou atualize para o ONTAP Mediator 1.9.1.
...
`ontap-mediator-1.9.1`
 a| 
O executável de instalação do produto para a versão 1.9.1
...
----
. Verifique os `ontap-mediator-1.9.1.sig.tsr` arquivos e `ontap-mediator-1.9.1.tsr` usando os certificados associados:
+
`openssl ts -verify`
