...
[NOTE]
====
* A partir de 15 de junho de 2025, você não poderá instalar ou atualizar para o ONTAP Mediator 1.8 e 1.9 porque os certificados de verificação de assinatura de código expiraram. Em vez disso, instale ou atualize para o ONTAP Mediator 1.10.
...
`ontap-mediator-1.10`
 a| 
O executável de instalação do produto para a versão 1.10
...
----
. Verifique o  `ontap-mediator-1.10.sig.tsr` e  `ontap-mediator-1.10.tsr` arquivos usando os certificados associados:
+
`openssl ts -verify`
