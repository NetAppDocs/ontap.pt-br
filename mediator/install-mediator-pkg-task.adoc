...
** Responda  `No`se você planeja usar ONTAP Mediator apenas para SnapMirror active sync ou se planeja configurar ONTAP Mediator com HTTPS para monitorar uma ou mais configurações de MetroCluster IP.
+
Alternativamente, você pode usar a  `--https-only` opção no  `./ontap-mediator-1.12.1/ontap-mediator-1.12.1 -y` comando para especificar somente HTTPS.
...
[NOTE]
====
Para ignorar a verificação de assinatura durante a instalação, use este comando: `./ontap-mediator-1.12.1/ontap-mediator-1.12.1 -y --skip-code-signature-check`
