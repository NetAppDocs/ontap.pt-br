...
* Suporte para configurar um ONTAP Mediator que usa HTTPS para comunicação em MetroCluster configurações IP. HTTPS oferece maior segurança ao criptografar sessões inteiras usando SSL/TLS. Você pode optar por instalar ONTAP Mediator que usa apenas o protocolo HTTPS. O pacote SCST não é instalado se este método de instalação for usado. link:index.html#https-iscsi-installation["Saiba mais sobre as opções de instalação HTTPS e iSCSI para ONTAP Mediator"].
