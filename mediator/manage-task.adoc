...
+
A autoridade PKI verifica a solicitação e assina o `.csr` , gerando o certificado `intermediate.crt` .  Além disso, você precisa obter o `root_ca.crt` certificado que assinou o `intermediate.crt` certificado da autoridade PKI.

+

NOTE: Para os clusters de SnapMirror Business Continuity (SM-BC) e as configurações de IP do MetroCluster usando o ONTAP Mediator configurado com HTTPS, você deve adicionar os  `intermediate.crt` e  `root_ca.crt` certificados a um cluster ONTAP. Consulte link:../snapmirror-active-sync/mediator-install-task.html["Configurar o ONTAP Mediator e os clusters para sincronização ativa do SnapMirror"] e link:https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/task_configuring_the_ontap_mediator_service_from_a_metrocluster_ip_configuration.html["Configure o ONTAP Mediator para uma configuração IP do MetroCluster"^].
...
[role="tabbed-block"]
====
.ONTAP Mediator 1.10 e posterior
--
...
. Atualizar o `/opt/netapp/lib/ontap_mediator/uvicorn/config.json` arquivo.
+
Atualizar os valores de `ssl_keyfile` , `ssl_certfile` , e `ssl_ca_certs` :
...
+
** O `ssl_keyfile` valor é o caminho da chave no `ontap_mediator_server.crt` arquivo, que é `ontap_mediator_server.key` .
** O `ssl_certfile` valor é o caminho do `ontap_mediator_server_chain.crt` arquivo.
** O `ssl_ca_certs` valor é o caminho do `root_ca.crt` arquivo.
...
----
+
.. Se você estiver usando uma estrutura de certificado onde o `root_ca.crt` certificado fornece um `intermediate.crt` certificado que assina o `ontap_mediator_server.crt` certificado, crie uma cadeia de certificados a partir do `intermediate.crt` e `ontap_mediator_server.crt` certificados:
+
...
NOTE: Para clusters SnapMirror Business Continuity (SM-BC), você deve adicionar o `intermediate.crt` e `root_ca.crt` certificados para um cluster ONTAP . Verlink:../snapmirror-active-sync/mediator-install-task.html["Configurar o ONTAP Mediator e os clusters para sincronização ativa do SnapMirror"] .
