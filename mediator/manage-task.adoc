...
[NOTE]
====
* No ONTAP Mediator 1,8 e anterior, os arquivos de CA usados são: `ca.key`, `ca.csr`, `ca.srl`, E `ca.crt`. A partir do ONTAP Mediator 1,9, os arquivos necessários são `intermediate.key`, `intermediate.csr`, `intermediate.srl` e `intermediate.crt`.
