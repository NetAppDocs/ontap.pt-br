...
ONTAP Mediator depende do SCST para fornecer iSCSI (consulte  http://scst.sourceforge.net/index.html[]). Este pacote é um módulo do kernel compilado durante a instalação especificamente para o kernel. Quaisquer atualizações do kernel podem exigir que o SCST seja reinstalado. Como alternativa, desinstale e reinstale o ONTAP Mediator e, em seguida, reconfigure o relacionamento ONTAP.


NOTE: Quaisquer atualizações no kernel do sistema operacional do servidor devem ser coordenadas com uma janela de manutenção no ONTAP.
