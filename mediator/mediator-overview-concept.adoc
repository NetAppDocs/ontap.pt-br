...
== Atualizações SCST e do kernel

ONTAP Mediator usa SCST para fornecer iSCSI para comunicação em MetroCluster configurações IP. O pacote SCST é um módulo do kernel que é compilado durante a instalação. Pode ser necessário reinstalar SCST após atualizações do kernel. Como alternativa, desinstale ONTAP Mediator, depois reinstale e reconfigure o relacionamento ONTAP. Consulte link:https://scst.sourceforge.net/["SCST: Um Subsistema de Destino SCSI Genérico para Linux"^] para saber mais sobre SCST.


NOTE: Você deve coordenar quaisquer atualizações no kernel do sistema operacional do servidor com uma janela de manutenção no ONTAP.



== Suporte para HTTPS e iSCSI

A partir do ONTAP Mediator 1.12, você pode configurar um ONTAP Mediator que usa HTTPS ou iSCSI para monitorar uma configuração de IP MetroCluster. Você pode optar por instalar ONTAP Mediator com suporte para ambos iSCSI e HTTPS ou somente HTTPS. Se ONTAP Mediator for instalado com suporte apenas para HTTPS, o pacote SCST não será instalado. Consulte link:index.html#https-iscsi-installation["Opções de instalação HTTPS e iSCSI para ONTAP Mediator"] para obter mais informações.
