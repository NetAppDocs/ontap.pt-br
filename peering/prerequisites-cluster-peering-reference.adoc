...
[role="lead"]
Antes de configurar o peering de cluster, você deve confirmar se os requisitos de conectividade, porta, endereço IP, sub-rede, firewall e cluster são atendidos.
...
== Limites de peer de cluster

O número de peers de cluster suportados depende da versão do ONTAP:

* ONTAP 9.14.1 e versões posteriores: até 400 peers de cluster são suportados.
* Versões anteriores do ONTAP: até 255 peers de cluster são suportados.
+
....
[NOTE]
