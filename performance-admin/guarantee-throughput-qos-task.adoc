...
summary: Você pode usar qualidade do serviço de storage para garantir que o desempenho de cargas de trabalho críticas não seja prejudicado por cargas de trabalho concorrentes. Você pode definir um teto de taxa de transferência para uma carga de trabalho concorrente para limitar seu impacto nos recursos do sistema, ou definir um piso de taxa de transferência para uma carga de trabalho crítica, garantindo que ela atenda às metas mínimas de taxa de transferência, independentemente da demanda de cargas de trabalho concorrentes. Você pode até mesmo definir um teto e um piso para a mesma carga de trabalho. 
---
= Saiba mais sobre a garantia de throughput de qualidade do serviço (QoS) do ONTAP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Você pode usar qualidade do serviço (QoS) de armazenamento para garantir que o desempenho de cargas de trabalho críticas não seja prejudicado por cargas de trabalho concorrentes. Você pode definir um limite máximo de throughput para uma carga de trabalho concorrente para limitar seu impacto nos recursos do sistema, ou definir um limite mínimo de throughput para uma carga de trabalho crítica, garantindo que ela atenda às metas mínimas de throughput, independentemente da demanda de cargas de trabalho concorrentes. Você pode definir um limite máximo de throughput e um limite mínimo de throughput para a mesma carga de trabalho.

Os grupos de políticas de qualidade do serviço adaptativos e não adaptativos são ativados automaticamente para aumentar temporariamente o teto de throughput de uma carga de trabalho em 50% por 1 segundo, atendendo a demandas de curto prazo por maior desempenho. Por exemplo, suponha que você esteja inicializando um banco de dados ou inicializando várias máquinas virtuais (VMs). Essas cargas de trabalho podem gerar um pico no consumo de recursos, causando a lentidão de outras cargas de trabalho. O aumento automático e temporário do throughput permite que o ONTAP aloque os recursos de curto prazo necessários para inicializar o banco de dados ou inicializar as máquinas virtuais sem a sua intervenção e sem que você incorra no custo de aumentar permanentemente os recursos do sistema para suportar essas tarefas. A partir do ONTAP 9.19.1, você pode usar as  `burst-percent`,  `burst-duration` e  `burst-iops` opções com o comando  `qos-create` e o comando  `qos-modify` para alterar esses valores padrão. No ONTAP 9.18.1 e versões anteriores, as configurações padrão não podem ser modificadas.
...
| Volume FlexGroup | sim | sim | sim | não 


| 1 | sim | sim | não | não 


| Vários workloads por grupo de políticas | sim | sim | sim | não 


| Grupos de políticas não compartilhados | sim | sim | sim | não 
|===
