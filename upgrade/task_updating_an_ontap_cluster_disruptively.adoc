...
---
= Atualize manualmente um cluster ONTAP padrão de forma disruptiva
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.Sobre esta tarefa
Você deve usar a interface de linha de comando do ONTAP (CLI) para realizar uma atualização manual com interrupção.
