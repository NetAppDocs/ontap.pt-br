...
---
= Realize uma atualização automatizada e não disruptiva do ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Ao executar uma atualização sem interrupções (ANDU), o ONTAP instala automaticamente a imagem ONTAP de destino em cada nó, valida se o cluster pode ser atualizado com sucesso e, em seguida, executa um xref:concept_upgrade_methods.html[atualização em lote ou contínua] em segundo plano, dependendo do número de nós no cluster.
...
IMPORTANT: Modificar a configuração da `storage failover modify-auto-giveback` opção de comando antes do início de uma ANDU não tem impacto no processo de atualização. O processo ANDU ignora qualquer valor predefinido para esta opção durante o takeover/giveback necessário para a atualização. Por exemplo, definir `-autogiveback` como falso antes de iniciar a ANDU não interrompe a atualização automática antes do giveback. Saiba mais sobre `storage failover modify-auto-giveback` na link:https://docs.netapp.com/us-en/ontap-cli/search.html?q=storage+failover+modify-auto-giveback["Referência do comando ONTAP"^].
...
* Opcionalmente, link:../update/pre-stage-system-firmware.html["Pré-configure o firmware do sistema para uma atualização com reinicialização única"].
+
A partir do ONTAP 9.17.1, se um BIOS ou firmware LOADER mais recente já estiver presente no dispositivo de inicialização de um nó antes de você executar um ANDU, o processo de atualização preserva o firmware mais recente em vez de sobrescrevê-lo com a versão incluída na imagem do ONTAP. Isso permite que tanto a atualização do software ONTAP quanto a atualização do firmware sejam concluídas em um único ciclo de reinicialização.
...
+
Para FTP anônimo, você deve especificar a URL no formato \ftp://anonymous@ftpserver.
...
|===
.. Selecione *Validar* para executar as verificações de validação pré-atualização.
+
...
+
Uma lista completa das verificações prévias automatizadas de atualização é exibida, juntamente com quaisquer erros ou avisos que devem ser resolvidos antes de iniciar o processo de atualização. Você deve resolver todos os erros antes de prosseguir com a atualização. É uma boa prática também resolver os avisos.
...
====
.O que se segue?
Se você pré-configurou o firmware, use o comando  `system node firmware bootmedia show` para verificar se a versão de firmware desejada está ativa em cada nó.
