...
---
= Verificações de configuração especiais antes de uma atualização do ONTAP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
| Tenho snapshots de retenção de longo prazo habilitados no volume do meio de uma cascata? | link:snapmirror-cascade-relationship-blocked.html["Desative snapshots de retenção de longo prazo em volumes intermediários de topologias em cascata"] 
