...
---
= NDU manual do ONTAP em uma configuração de quatro ou oito nós do MetroCluster
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Uma atualização manual não disruptiva (NDU) de uma configuração de quatro ou oito nós do MetroCluster envolve a preparação para a atualização, a atualização simultânea dos pares de DR em cada um dos um ou dois grupos de DR e a execução de tarefas pós-atualização.

.Sobre esta tarefa
* Você deve usar a interface de linha de comando do ONTAP (CLI) para executar esta tarefa.
...
----
+
O exemplo a seguir mostra que a system image2 (imagem ONTAP de destino) é a versão padrão e atual no node_A_1:
...
----
+
O exemplo a seguir mostra que a imagem do sistema image2 (imagem ONTAP de destino) é a versão padrão e atual no node_B_1:
...
----
+
O exemplo a seguir mostra que a imagem do sistema image2 (imagem ONTAP de destino) é a versão padrão e atual no node_A_2:
...
----
+
O exemplo a seguir mostra que a system image2 (imagem ONTAP de destino) é a versão padrão e atual no node_B_2:
