...
summary: O desempenho das cargas de trabalho começa a ser afetado pela latência quando a quantidade de trabalho em um nó excede os recursos disponíveis. Você pode gerenciar um nó sobrecarregado aumentando os recursos disponíveis, como atualizando discos ou CPU, ou reduzindo a carga, como movendo volumes ou LUNs para nós diferentes conforme necessário. 
---
= Saiba mais sobre balanceamento de carga no ONTAP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
