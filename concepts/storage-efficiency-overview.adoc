...
---
= Saiba mais sobre a eficiência de storage do ONTAP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
A eficiência de armazenamento sensível à temperatura (TSSE), introduzida no ONTAP 9.8, é ativada automaticamente em volumes AFF recém-criados e com provisionamento dinâmico. Não está habilitado emlink:builtin-storage-efficiency-concept.html["Plataformas AFF A70, AFF A90 e AFF A1K"] que foram introduzidas no ONTAP 9.15.1, as quais utilizam um processador de descarregamento de hardware.




NOTE: O TSSE não é suportado apenas em volumes com provisionamento espesso.
