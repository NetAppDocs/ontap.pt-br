...
.Passos
. No System Manager, selecione *Armazenamento > Volumes*.
+
É apresentada a lista de volumes.

+
Os administradores do SVM veem um conjunto limitado de opções e podem não ter permissão para adicionar volumes.
...
Adicione um volume
 a| 
Selecione image:icon_add_blue_bg.gif["Adicionar ícone"]. Consulte link:../task_admin_add_a_volume.html["Adicione um volume"].
...
* Selecione image:icon_delete_with_can_white_bg.gif["Eliminar ícone"] para excluir os volumes selecionados.
* Selecione image:icon_protect.gif["Ícone proteger"] para atribuir uma política de proteção aos volumes selecionados.
* Selecione image:icon-more-kebab-white-bg.gif["ícone mais"] para selecionar uma das seguintes ações a serem executadas em todos os volumes selecionados:
+
** Ativar quota
** Fique offline
** Mover
...
Gerenciar um único volume
 a| 
Ao lado do volume, selecione image:icon_kabob.gif["Ícone de opções do menu"] e, em seguida, selecione uma das seguintes ações para executar:
...
Selecione image:icon-edit-pencil-blue-outline.png["Ícone de edição"] ao lado do nome do volume e, em seguida, modifique o nome do volume.
