...
---
= Proteção contra exclusão acidental de volumes ONTAP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: Excluir a SVM que contém o volume excluído limpa a Volume Recovery Queue (VRQ). Exclua uma SVM somente quando tiver certeza de que não há necessidade de recuperar volumes pertencentes à SVM. Volumes na volume recovery queue não podem existir quando a SVM proprietária é excluída.
