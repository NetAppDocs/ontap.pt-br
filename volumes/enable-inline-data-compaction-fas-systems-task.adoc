...
summary: Habilite a compactação de dados inline em sistemas FAS com agregados Flash Pool ou agregados HDD no nível de volume ou agregado. 
---
...
[role="lead"]
Você pode habilitar a compactação de dados inline em sistemas FAS com agregados Flash Pool (híbridos) ou agregados HDD no nível do volume usando o comando do shell do cluster  `volume efficiency`. A compactação de dados está desabilitada por padrão para volumes criados em sistemas FAS. Saiba mais sobre  `volume efficiency` na link:https://docs.netapp.com/us-en/ontap-cli/search.html?q=volume+efficiency["Referência do comando ONTAP"^].
