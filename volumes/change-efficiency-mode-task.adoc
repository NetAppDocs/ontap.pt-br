...
[role="lead"]
A partir do ONTAP 9.10.1, os modos de eficiência de armazenamento em nível de volume _default_ e _efficient_ são suportados apenas para sistemas AFF . Esses modos oferecem a opção entre compressão de arquivos (padrão), que é o modo padrão ao criar novos volumes AFF , ou eficiência de armazenamento sensível à temperatura (eficiente), que ativa a eficiência de armazenamento sensível à temperatura.
