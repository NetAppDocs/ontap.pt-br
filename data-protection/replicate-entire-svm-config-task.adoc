...
* A partir do ONTAP 9.19.1, a ativação do IPsec não é suportada em SVMs configuradas para SVM DR. Se você estiver usando IPsec, você deve desativá-lo ou escolher uma SVM diferente para configurar um relacionamento SVM DR. Você receberá um erro se estiver usando ONTAP 9.19.1 ou posterior e tentar ativar o IPsec em uma SVM configurada para SVM DR.
...
. A partir do cluster de origem, crie uma relação de pares SVM usando o comando  `vserver peer create`.
+
