...
| Replicação Snapshot criada pela aplicação | ONTAP 9,7 | Se um snapshot for marcado com o rótulo apropriado no momento da  `snapshot create` operação, usando a CLI ou a ONTAP API, SnapMirror síncrono replica os snapshots, tanto os criados pelo usuário quanto aqueles criados com scripts externos, após a suspensão dos aplicativos. A partir do ONTAP 9.14.1, o requisito de rótulo para replicação de snapshots criados por aplicativos não existe mais. Para obter mais informações sobre a replicação de snapshots criados por aplicativos, consulte a link:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Protection_and_Security/SnapMirror/How_to_replicate_application_created_snapshots_with_SnapMirror_Synchronous["Base de conhecimento da NetApp : Como replicar snapshots criados por aplicativos com o SnapMirror síncrono"^]. 


| Proteção autônoma contra ransomware | ONTAP 9.19.1 |  
...
| Restauração NDMP/NDMP | ONTAP 9.13,1 | Tanto o cluster de origem quanto o cluster de destino devem estar executando o ONTAP 9.13.1 ou posterior para realizar backup NDMP baseado em dump ou SMTape de volumes síncronos do SnapMirror. As restaurações NDMP, baseadas em dump ou SMTape, não são compatíveis com volumes de destino em uma relação de SnapMirror active sync ou síncrona do SnapMirror. 
...
| Replicação de operações de gerenciamento de snapshots | ONTAP 9.19.1 | A partir do ONTAP 9.19.1, as operações de gerenciamento de snapshots `snapshot delete`, `snapshot rename` e `snapshot modify` são replicadas para o cluster secundário. 


| Replicação de Snapshot agendada | ONTAP 9.19.1 | A partir do ONTAP 9.19.1, é possível habilitar a replicação de snapshots agendados usando uma política SnapMirror. Em versões anteriores ao ONTAP 9.19.1, os snapshots agendados criados usando uma política de snapshot não são replicados. 
...
| Recuperação de desastres da SVM | ONTAP 9,6  a| 
* Uma fonte síncrona de SnapMirror também pode ser uma fonte de recuperação de desastres SVM, por exemplo, uma configuração de ramificação com SnapMirror síncrono em uma das extremidades e recuperação de desastres SVM na outra.
* Uma origem síncrona do SnapMirror não pode ser um destino de recuperação de desastres do SVM porque o SnapMirror síncrono não oferece suporte ao encadeamento de uma origem de proteção de dados. Você deve liberar o relacionamento síncrono antes de executar uma resincronização de inversão de recuperação de desastres do SVM no cluster de destino.
* Um destino SnapMirror síncrono não pode ser uma origem de recuperação de desastres do SVM porque a recuperação de desastres do SVM não oferece suporte à replicação de volumes DP. Uma resincronização reversa da origem síncrona faria com que a recuperação de desastres do SVM excluísse o volume DP no cluster de destino.




| link:../snaplock/snapshot-lock-concept.html["Snapshots à prova de violações"] | ONTAP 9.19.1 | Você só pode especificar o tempo de expiração durante a criação do snapshot. Não é possível definir ou estender o tempo de expiração em snapshots já existentes. 
...
* SnapCenter
* Volumes SnapLock
* Restauração NDMP baseada em dump ou SMTape para volumes de destino em um relacionamento de sincronização ativa do SnapMirror ou relacionamento síncrono do SnapMirror
...
== Replicação de Snapshot agendada

A partir do ONTAP 9.19.1, você pode usar o parâmetro de política do SnapMirror `-sync-snapshot-replication-mode` no modo de privilégio avançado para replicar snapshots programados para o destino do SnapMirror. A replicação de snapshots programados fornece simetria entre os volumes primário e secundário em uma relação síncrona do SnapsMirror.

.Pré-requisitos
* Você precisa ser um administrador de cluster ou SVM
* Você precisa estar no modo de privilégios avançados
* Você deve configurar uma política SnapMirror definida pelo usuário que utilize o parâmetro  `-sync-snapshot-replication-mode` e inclua a opção  `sched_created`.
+
** As políticas definidas pelo sistema não podem ser modificadas. Você deve criar uma nova política e restabelecer o relacionamento.
** As alterações de política definidas pelo usuário exigem uma resincronização (quiesce seguida de retomada) para entrarem em vigor nos relacionamentos existentes.




O parâmetro aceita estas opções:

* `none`- desativa a replicação de snapshots síncronos que não são do SnapMirror
* `application_created`- replica snapshots criados pelo aplicativo
* `sched_created` - replica snapshots agendados
* `sm_created` - replica snapshots criados de forma assíncrona (ramo de fanout) do SnapMirror
* Uma lista separada por vírgulas de tipos: `application_created`, `sched_created` e `sm_created`.


O parâmetro só entra em vigor quando o status do relacionamento for  `InSync`. Tipos de Snapshot não listados são armazenados apenas no volume primário.

A partir do ONTAP 9.19.1, `snapmirror initialize` replica todos os snapshots para o cluster secundário, enquanto `snapmirror resync` apenas replica novos snapshots criados após o último snapshot comum. Durante uma operação de ressincronização, snapshots não replicados marcados como somente primário quando o status da relação é `InSync` não são replicados para o cluster secundário, e esses snapshots são mantidos no novo cluster secundário após uma operação de ressincronização de inversão.


NOTE: A criação de snapshots em volumes síncronos do SnapMirror pode aumentar a latência de I/O do usuário. Recomenda-se o uso de agendamentos de snapshots com uma frequência superior a 30 minutos.

.Informações relacionadas
* https://www.netapp.com/pdf.html?item=/media/17174-tr4733.pdf["Relatório técnico da NetApp 4733: Configuração síncrona da SnapMirror e práticas recomendadas"^]
* link:https://docs.netapp.com/us-en/ontap-cli//volume-snapshot-create.html["criar snapshot de volume"^]
* link:https://docs.netapp.com/us-en/ontap-cli/volume-snapshot-delete.html["excluir snapshot de volume"^]
* link:https://docs.netapp.com/us-en/ontap-cli/volume-snapshot-modify.html["modificar snapshot de volume"^]
* link:https://docs.netapp.com/us-en/ontap-cli/volume-snapshot-rename.html["renomear snapshot de volume"^]
