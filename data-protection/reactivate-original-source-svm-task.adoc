...
--
. No cluster de destino, selecione a relação de proteção desejada: clique em *Proteção > Replicação*.
. Posicione o cursor sobre o nome da fonte e clique. image:icon_kabob.gif["Ícone de opções do menu"]e clique em *Reverter resincronização*.
...
. No cluster de origem, clique em *Proteção > Replicação* e verifique se a resincronização reversa foi concluída, conferindo se o estado exibe *Espelhado*.
. No cluster de destino original, navegue até *Cluster > VMs de armazenamento*.
. Localize a VM de armazenamento, passe o cursor sobre o nome da VM de armazenamento e clique. image:icon_kabob.gif["Ícone de opções do menu"]e clique em *Parar*.
. Na janela *Parar VM de armazenamento*, clique em *Parar*.
. No cluster de origem, navegue até *Proteção > Replicação* e localize a VM de armazenamento que deseja reativar. Posicione o cursor sobre o nome da VM de armazenamento e clique. image:icon_kabob.gif["Ícone de opções do menu"]e clique em *Ativar VM de armazenamento de destino*.
. Na janela *Ativar VM de armazenamento de destino*, selecione *Ativar a VM de armazenamento de destino e desfazer a relação* e clique em *Ativar*.
. Ao retornar à página *Replicação*, passe o cursor novamente sobre o nome da VM de armazenamento e clique. image:icon_kabob.gif["Ícone de opções do menu"]e clique em *Reverter resincronização*.


--
.Gerenciador de sistemas ONTAP 9.16.1 e versões anteriores
--
. No cluster de destino, selecione a relação de proteção desejada: clique em *Proteção > Relações*.
. Posicione o cursor sobre o nome da fonte e clique. image:icon_kabob.gif["Ícone de opções do menu"]e clique em *Reverter resincronização*.
. Na janela *Reverse Resync relation*, clique em *Reverse Resync*.
+
O relacionamento desaparece da tabela *Relationships* porque agora está sendo gerenciado pelo cluster de origem original.

. No cluster de origem, clique em *Proteção > Relacionamentos* e verifique se a resincronização reversa foi concluída, conferindo se o estado está como *Espelhado*.
. No cluster de destino original, navegue até *Armazenamento > VMs de armazenamento*.
. Localize a VM de armazenamento, passe o cursor sobre o nome da VM de armazenamento e clique. image:icon_kabob.gif["Ícone de opções do menu"]e clique em *Parar*.
. Na janela *Parar VM de armazenamento*, clique em *Parar*.
. No cluster de origem, localize a VM de armazenamento (que agora é a SVM de origem da relação inversa), passe o cursor sobre o nome da SVM e clique. image:icon_kabob.gif["Ícone de opções do menu"]e clique em *Ativar VM de armazenamento de destino*.
. Na janela *Ativar VM de armazenamento de destino*, selecione *Ativar a VM de armazenamento de destino e desfazer a relação* e clique em *Ativar*.
. Ao retornar à página *Relacionamentos*, passe o cursor novamente sobre o nome da VM de armazenamento e clique. image:icon_kabob.gif["Ícone de opções do menu"]e clique em *Reverter resincronização*.
