...
summary: Para todos os tipos de relacionamento, a inicialização realiza uma transferência completa. Ela cria um instantâneo do volume de origem, depois transfere essa cópia e todos os blocos de dados que ela referencia para o volume de destino. Caso contrário, o conteúdo da transferência depende da política. 
---
...
Você deve estar ciente de que se um sistema de arquivos for reinicializado por qualquer motivo, como reinicialização de nó, aquisição/devolução ou pânico, a inicialização não será retomada automaticamente e deverá ser reiniciada manualmente.
