...
summary: O monitoramento de integridade fornece informações sobre o status de integridade do sistema em todo o cluster. O monitor de integridade monitora Hyper-V e SQL Server sobre configurações SMB para garantir operações ininterruptas para os servidores de aplicativos. Se o status estiver degradado, você pode visualizar detalhes sobre o problema, incluindo a causa provável e as ações de recuperação recomendadas. 
---
