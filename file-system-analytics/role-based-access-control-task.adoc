...
[role="lead"]
A partir do ONTAP 9.12.1, o ONTAP inclui uma função de controle de acesso baseado em funções (RBAC) predefinida chamada  `admin-no-fsa`. A função  `admin-no-fsa` concede privilégios de nível de administrador, mas impede que o usuário execute operações relacionadas ao endpoint  `files` (ou seja, File System Analytics) na CLI do ONTAP, na API REST do ONTAP e no System Manager.
