...
* O cluster de destino deve estar no mesmo ou não mais do que duas versões mais recentes do cluster de origem (ECV).
...
A migração do SVM é compatível com volumes no FabricPools para as plataformas a seguir:

* Plataforma Azure NetApp Files. Todas as políticas de disposição em categorias são compatíveis (somente snapshot, automático, all e nenhum).
...
| SVMs MetroCluster | Não suportado | Embora a migração do SVM não seja compatível com a migração do MetroCluster SVM, você pode usar a replicação assíncrona do SnapMirror para link:https://www.netapp.com/media/83785-tr-4966.pdf["Migrar um SVM em uma configuração do MetroCluster"]o . Você deve estar ciente de que o processo descrito para migrar um SVM em uma configuração do MetroCluster é _não_ um método sem interrupções. 


| Criptografia de agregados NetApp (NAE) | Não suportado | A migração não é suportada de uma fonte não encriptada para um destino encriptado. 
...
[NOTE]
====
O redirecionamento de log de auditoria só está disponível no modo de nuvem. Para a migração SVM no local com auditoria habilitada, você deve desativar a auditoria na SVM de origem e, em seguida, executar a migração.
...
| Relacionamentos de nuvem da SnapMirror | ONTAP 9.12,1 | A partir do ONTAP 9.12,1, quando você migra um SVM com relacionamentos de nuvem SnapMirror, o cluster de destino precisa ter o link:../data-protection/snapmirror-licensing-concept.html#snapmirror-cloud-license["Licença de nuvem da SnapMirror"] instalado e a capacidade suficiente disponível para dar suporte à migração de capacidade nos volumes espelhados para a nuvem. 
...
| Cópia Snapshot | ONTAP 9.10,1 |  


| Bloqueio de cópias Snapshot à prova de violações | ONTAP 9.14,1 | O bloqueio de cópias Snapshot à prova de violações não é equivalente ao SnapLock. O SnapLock permanece sem suporte. 


| Virtual IP LIFs/BGP | Não suportado |  


| Console de armazenamento virtual 7,0 e posterior | Não suportado | O VSC faz parte https://docs.netapp.com/us-en/ontap-tools-vmware-vsphere/index.html["Ferramentas do ONTAP para o dispositivo virtual VMware vSphere"^] do início com o VSC 7,0. 
...
| Os atributos de cópia Snapshot modificam | Permitido | Permitido | Não suportado 


| Cópia snapshot autodelete Modificar | Permitido | Permitido | Não suportado 


| Cópia Snapshot criar | Permitido | Permitido | Não suportado 


| Eliminar cópia Snapshot | Permitido | Permitido | Não suportado 


| Restaure o arquivo a partir da cópia Snapshot | Permitido | Permitido | Não suportado 
|===