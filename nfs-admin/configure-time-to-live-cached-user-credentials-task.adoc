...
summary: Você pode configurar o período de tempo que o ONTAP armazena as credenciais dos usuários NFS em seu cache interno, modificando o servidor NFS da máquina virtual de storage. Isso permite atenuar certos problemas relacionados à alta carga nos servidores de nomes ou a alterações nas credenciais que afetam o acesso do usuário NFS. 
---
...
O TTL é medido em milissegundos. A partir do ONTAP 9.17.1 e versões posteriores, o padrão é 1 hora (3.600.000 milissegundos). No ONTAP 9.16.1 e versões anteriores, o padrão é 24 horas (86.400.000 milissegundos). O intervalo permitido para esse valor é de 1 minuto (60.000 milissegundos) até 7 dias (604.800.000 milissegundos).
