...
summary: Você pode exibir informações sobre os volumes montados para máquinas virtuais de armazenamento e os pontos de junção aos quais os volumes estão montados. Você também pode determinar quais volumes não estão montados em um ponto de junção. Você pode usar essas informações para entender e gerenciar seu namespace SVM. 
---
