...
summary: Você pode configurar BranchCache para compartilhamentos individuais ou para todos os compartilhamentos SMB. Se escolher a configuração por compartilhamento, você deve habilitar BranchCache em cada compartilhamento SMB que deve fornecer armazenamento em cache. 
---
...
[role="lead"]
Para usar o BranchCache, você deve configurar seus clientes e, se necessário, os servidores de cache hospedados nas filiais onde o conteúdo será armazenado em cache. Você pode configurar o BranchCache para compartilhamentos individuais ou para todos os compartilhamentos SMB. Se optar pela configuração por compartilhamento, você deve habilitar o BranchCache em cada compartilhamento SMB que deve fornecer armazenamento em cache. Você também pode configurar o BranchCache para habilitar o armazenamento em cache automaticamente em todos os compartilhamentos SMB.
...
----
.. Verifique as configurações do SVM SMB para determinar se todas as versões necessárias do SMB estão habilitadas:
+
[source, cli]
----
vserver cifs options show -vserver <SVM_name>
----
.. Se necessário, habilite o SMB 2.1:
+
[source, cli]
----
...
.. Se necessário, habilite o SMB 3.0:
+
[source, cli]
----
