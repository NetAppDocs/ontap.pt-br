...
summary: A permissão para ignorar a verificação de percurso é um direito do usuário que determina se um usuário pode percorrer todos os diretórios no caminho até um arquivo, mesmo que não tenha permissões no diretório percorrido. Você deve entender o que acontece ao permitir ou negar a verificação de percurso ignorada e como configurar a verificação de percurso ignorada para usuários em storage virtual machines. 
---
...
* xref:assign-privileges-concept.html[Lista de Privileges suportados]
