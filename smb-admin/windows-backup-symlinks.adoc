...
summary: Quando um aplicativo de backup em execução no Windows encontra um link simbólico no estilo Unix, o link é seguido e os dados são copiados. A partir do ONTAP 9.15.1, você tem a opção de fazer backup dos links simbólicos em vez dos dados. Esse recurso é totalmente compatível com volumes FlexGroup e FlexVols do ONTAP. 
---
...
Além da CLI do ONTAP, você também pode gerenciar esse recurso usando a API REST do ONTAP. Consulte  https://docs.netapp.com/us-en/ontap-automation/whats-new.html["Novidades com a API REST e a automação do ONTAP"^] para mais informações. A configuração que determina como o ONTAP processa os links simbólicos no estilo Unix deve ser realizada separadamente para cada SVM.
