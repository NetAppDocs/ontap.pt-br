...
summary: Existem vários motivos para criar usuários e grupos locais em sua máquina virtual de storage. Por exemplo, você pode acessar um servidor SMB usando uma conta de usuário local caso os controladores de domínio estejam indisponíveis, pode ser útil usar grupos locais para atribuir privilégios ou seu servidor SMB pode estar em um grupo de trabalho. 
---
