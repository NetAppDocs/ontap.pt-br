...
summary: Se você tiver apenas clientes CIFS em seu ambiente e sua máquina virtual de storage tiver sido configurada como um sistema de storage multiprotocolo, você deverá ter pelo menos uma conta do Windows com privilégios de root para acessar arquivos na SVM. Caso contrário, você não poderá gerenciar a SVM porque não possui direitos de usuário suficientes. 
---
