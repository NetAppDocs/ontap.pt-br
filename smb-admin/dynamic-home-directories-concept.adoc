...
summary: Os diretórios base do ONTAP permitem configurar um compartilhamento SMB que mapeia diferentes diretórios com base no usuário que se conecta a ele e em um conjunto de variáveis. Em vez de criar compartilhamentos separados para cada usuário, você pode configurar um único compartilhamento com alguns parâmetros de diretório base para definir a relação de um usuário entre um ponto de entrada e o diretório base. 
---
