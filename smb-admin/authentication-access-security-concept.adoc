...
summary: Antes que os usuários possam criar conexões SMB para acessar os dados contidos na SVM, eles devem ser autenticados pelo domínio ao qual o servidor SMB pertence. O servidor SMB suporta dois métodos de autenticação, Kerberos e NTLM. Kerberos é o método padrão usado para autenticar usuários do domínio. 
---
