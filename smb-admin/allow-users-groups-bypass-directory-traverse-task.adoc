...
summary: Se você deseja que um usuário possa percorrer todos os diretórios no caminho para um arquivo, mesmo que o usuário não tenha permissões em um diretório percorrido, você pode adicionar o privilégio SeChangeNotifyPrivilege a usuários ou grupos SMB locais em máquinas virtuais de armazenamento. Por padrão, os usuários podem ignorar a verificação de percurso de diretório. 
---
