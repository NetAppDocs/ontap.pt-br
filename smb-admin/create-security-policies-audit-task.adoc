...
summary: Criar uma política de auditoria para máquinas virtuais de armazenamento é o terceiro passo na configuração e aplicação de ACLs a um arquivo ou pasta. Uma política funciona como um contêiner para várias tarefas, onde cada tarefa é uma entrada individual que pode ser aplicada a arquivos ou pastas. Você pode adicionar tarefas à política de segurança posteriormente. 
---
