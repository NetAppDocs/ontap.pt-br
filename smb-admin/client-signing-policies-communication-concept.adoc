...
[NOTE]
====
A Microsoft não recomenda mais o uso de  `Digitally sign communications (if client agrees)` ou  `Digitally sign communications (if server agrees)` configurações de Política de Grupo. A Microsoft também não recomenda mais o uso das configurações de registro  `EnableSecuritySignature`. Essas opções afetam apenas o comportamento do SMB 1 e podem ser substituídas pela configuração de Política de Grupo  `Digitally sign communications (always)` ou pela configuração de registro  `RequireSecuritySignature`. Você também pode obter mais informações em link:http://blogs.technet.com/b/josebda/archive/2010/12/01/the-basics-of-smb-signing-covering-both-smb1-and-smb2.aspx["Blog da Microsoft: noções básicas de assinatura SMB (abrangendo SMB1 e SMB2)"^].
