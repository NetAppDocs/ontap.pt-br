...
summary: BranchCache foi desenvolvido pela Microsoft para permitir o armazenamento em cache de conteúdo em computadores locais aos clientes solicitantes. A implementação do BranchCache no ONTAP pode reduzir a utilização da rede de longa distância e proporcionar tempo de resposta aprimorado quando usuários em uma filial acessam conteúdo armazenado em máquinas virtuais de armazenamento usando SMB. 
---
