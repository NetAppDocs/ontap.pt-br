...
summary: A partir do ONTAP 9, você pode configurar a assinatura e a selagem para habilitar a segurança da sessão LDAP em consultas a um servidor Active Directory. Você deve configurar as definições de segurança do servidor CIFS na máquina virtual de storage para que correspondam às do servidor LDAP. 
---
