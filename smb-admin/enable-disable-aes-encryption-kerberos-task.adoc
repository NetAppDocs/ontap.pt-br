...
summary: Para aproveitar a segurança mais robusta com a comunicação baseada em Kerberos, você pode habilitar a criptografia AES-256 e AES-128 no servidor SMB. Se você não quiser que o servidor SMB selecione os tipos de criptografia AES para a comunicação baseada em Kerberos com o KDC do Active Directory, você pode desabilitar a criptografia AES. Por padrão, a criptografia AES está desabilitada. 
---
...
Ao modificar o  `-advertised-enc-types`parâmetro, os tipos de criptografia especificados substituem quaisquer tipos configurados anteriormente. Todas as configurações anteriores de tipo de criptografia são removidas.

Para aumentar a segurança, a máquina virtual de armazenamento (SVM) altera a senha da conta de máquina no AD sempre que a opção de segurança AES é modificada. Alterar a senha pode exigir credenciais administrativas do AD para a unidade organizacional (OU) que contém a conta de máquina. Uma solicitação de credencial aparece somente quando o ONTAP precisa redefinir a senha da conta de máquina no Active Directory e sua configuração do AD exige credenciais administrativas para a unidade organizacional que contém a conta de máquina. Se o seu AD estiver sincronizado e a redefinição automática estiver habilitada, nenhuma solicitação será exibida.
