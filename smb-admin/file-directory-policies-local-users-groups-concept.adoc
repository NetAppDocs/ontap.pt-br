...
summary: Existem algumas diretrizes que você deve ter em mente antes de aplicar políticas de diretório de arquivos na máquina virtual de storage de destino de recuperação de desastres em uma configuração de descarte de ID, caso sua configuração de política de diretório de arquivos utilize usuários ou grupos locais no descritor de segurança ou nas entradas DACL ou SACL. 
---
