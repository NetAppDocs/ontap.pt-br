...
summary: Você pode exibir informações sobre a segurança de arquivos e diretórios contidos em volumes em máquinas virtuais de armazenamento. Você pode exibir informações sobre as políticas de auditoria em volumes FlexVol. Se configurado, você pode exibir informações sobre as configurações de segurança do Storage-Level Access Guard e do Dynamic Access Control em volumes FlexVol. 
---
