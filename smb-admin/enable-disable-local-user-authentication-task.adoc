...
summary: Você pode habilitar ou desabilitar a autenticação de usuário local para acesso SMB em máquinas virtuais de armazenamento. A configuração padrão é permitir a autenticação de usuário local, o que é útil quando a máquina virtual de armazenamento não consegue contatar um controlador de domínio ou se você optar por não usar controles de acesso em nível de domínio. 
---
