...
summary: 'Seu servidor SMB oferece suporte a Objetos de Política de Grupo (GPOs), um conjunto de regras conhecidas como atributos de política de grupo que se aplicam a computadores em um ambiente do Active Directory. Você pode usar GPOs para gerenciar centralmente as configurações de todas as máquinas virtuais de armazenamento no cluster pertencentes ao mesmo domínio do Active Directory.' 
---
