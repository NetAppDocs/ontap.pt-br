...
summary: 'Você pode exibir informações sobre a configuração do Objeto de Política de Grupo (GPO) do servidor CIFS para determinar se os GPOs do BranchCache estão definidos para o domínio ao qual o servidor CIFS pertence e, em caso afirmativo, quais são as configurações permitidas. Você também pode determinar se as configurações de GPO do BranchCache estão sendo aplicadas ao servidor CIFS.' 
---
