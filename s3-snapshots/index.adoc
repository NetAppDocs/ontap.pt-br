...
Os snapshots do S3 capturam apenas as versões atuais dos objetos nos buckets do S3. As versões não atuais dos buckets versionados não são capturadas nos snapshots do S3. Além disso, as tags de objeto de ponto no tempo não são capturadas nos snapshots se as tags de objeto forem modificadas após a captura dos snapshots.
...
Você deve estar ciente das seguintes exclusões de recursos e cenários:
