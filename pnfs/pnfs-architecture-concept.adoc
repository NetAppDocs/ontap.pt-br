...
NOTE: Nesses comandos, os nomes dos volumes não estão presentes. Em vez disso, são usados os IDs numéricos associados a esses volumes: o MSID e o DSID. Para encontrar os volumes associados aos mapeamentos, você pode usar  `volume show -dsid [dsid_numeric]` ou  `volume show -msid [msid_numeric]` no privilégio avançado da CLI do ONTAP.
...
* https://www.netapp.com/pdf.html?item=/media/12385-tr4571.pdf["Relatório Técnico 4571 da NetApp : Melhores Práticas do FlexGroup"^]
