...
NOTE: Você pode criar um volume FlexCache como cache para um volume de origem que tenha o recurso de camadas FabricPool ativado, mas o próprio volume FlexCache não pode ser dividido em camadas.
...
Compatível a partir do ONTAP 9.18.1
...
Não
 ...
 a| 
SnapMirror relações assíncronas
 a| 
Sim

Mais sobre as origens do FlexCache:
...
Compatível a partir do ONTAP 9.5. O SVM primário de um relacionamento SVM DR pode ter o volume de origem; no entanto, se você estiver executando uma versão do ONTAP anterior à ONTAP , quando o relacionamento SVM DR for interrompido, o relacionamento FlexCache deverá ser recriado com um novo volume de origem.

A partir do ONTAP 9.18.1, quando uma SVM de origem sofre uma falha de recuperação, os caches são automaticamente transferidos para a origem no site de recuperação de desastres (DR). As etapas de recuperação manual são eliminadas.

xref:create-volume-task.adoc[Aprenda a criar volumes FlexCache .]
 a| 
Não

Você pode ter volumes de origem FlexCache em SVMs primárias, mas não em SVMs secundárias. O relacionamento FlexCache não é replicado, mas os dados do volume de origem são replicados como um volume regular.
...
NOTE: Nas versões do ONTAP 9 anteriores à 9.5, volumes de origem FlexVol só podem fornecer dados para volumes FlexCache criados em sistemas que executam Data ONTAP 8.2.x operando em 7-Mode. A partir do ONTAP 9.5, volumes de origem FlexVol também podem fornecer dados para volumes FlexCache em sistemas ONTAP 9. Para obter informações sobre a migração de FlexCache 7-Mode para FlexCache ONTAP 9, consulte link:https://www.netapp.com/pdf.html?item=/media/7336-tr4743.pdf["Relatório Técnico da NetApp 4743: FlexCache em ONTAP"^].
