...
.Antes de começar
Deve existir um LUN de produção e ele deve estar mapeado para um igroup que inclua o WWPN ou o nome do nó do servidor de aplicativos. O LUN também deve estar formatado e acessível ao host.

.Passos
. Salve o conteúdo dos buffers do sistema de arquivos host no disco.
+
Você pode usar o comando fornecido pelo sistema operacional. Você também pode optar por incluir esta etapa no seu script de pré-processamento de backup SAN.
