...
summary: ONTAP oferece suporte a determinadas APIs vStorage da VMware para recursos de integração de arrays quando o host ESX estiver executando o ESX 4.1 ou posterior. Esses recursos ajudam a descarregar operações do host ESX para o sistema de storage e a aumentar a taxa de transferência da rede. O host ESX habilita os recursos automaticamente no ambiente correto. 
---
