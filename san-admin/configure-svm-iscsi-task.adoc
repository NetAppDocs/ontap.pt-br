...
summary: Para configurar uma máquina virtual de storage para iSCSI, você deve criar LIFs para a SVM e atribuir o protocolo iSCSI a essas LIFs. 
---
...
* A partir do ONTAP 9.19.1, sistemas AFF suportam multipath ativo-ativo com failover automático de LIF no nível do SVM em ambientes SAN. Você deve habilitar o multipath ativo-ativo em sistemas AFF ao criar o SVM pela primeira vez.
...
. Selecione *Habilitar iSCSI* e insira o endereço IP e a máscara de sub-rede para a interface de rede.
+
...
+
Esta opção está disponível no ONTAP 9.19.1 e versões posteriores apenas em sistemas AFF e ASA.

+
** Para sistemas AFF, selecione *Multipathing Ativo-Ativo Simétrico* e, em seguida, selecione *Ativar Multipathing Ativo-Ativo Simétrico*.
** Para sistemas ASA, selecione *Multipathing Ativo-Ativo Simétrico* e, em seguida, selecione *Habilitar para protocolos SCSI e NVMe*.
...
. Selecione a guia *Configurações* e, em seguida, selecione image:icon_gear.gif["Ícone ações"] ao lado do protocolo iSCSI.
. Selecione *Habilitar iSCSI* e insira o endereço IP e a máscara de sub-rede para a interface de rede. + Cada nó deve ter pelo menos duas interfaces de rede.
. Selecione *Guardar*.
