...
[role="lead"]
Ao provisionar armazenamento, você habilita seus hosts NVMe para ler e gravar dados em sistemas de armazenamento ONTAP. Para sistemas que utilizam o protocolo NVMe, você deve criar um ou mais namespaces e subsistemas NVMe. Cada namespace pode então ser mapeado para um subsistema NVMe para permitir o acesso aos dados a partir do seu host.

Se a máquina virtual de armazenamento (SVM) que contém o subsistema NVMe tiver interfaces de rede NVMe/FC e NVMe/TCP, os namespaces mapeados para o subsistema NVMe poderão ser acessados por ambos os protocolos de transporte NVMe/FC e NVMe/TCP. O suporte a NVMe para FC está disponível a partir do ONTAP 9.4 e versões posteriores. O suporte a NVMe para TCP está disponível a partir do ONTAP 9.10.1 e versões posteriores. Se você precisar de acesso específico por protocolo aos seus namespaces, deverá configurar link:configure-svm-nvme-task.html["configurar uma SVM"] para cada protocolo.
