...
summary: A Transferência de Dados Descarregada da Microsoft, também conhecida como descarregamento de cópia, permite transferências diretas de dados dentro de um dispositivo de storage ou entre dispositivos de storage compatíveis, sem transferir os dados pelo computador host. 
---
...
Ao usar o recurso de descarregamento de cópia do VMware e da Microsoft em ambientes virtualizados, seus LUNs devem estar alinhados.  LUNs desalinhadas podem degradar o desempenho. link:io-misalignments-properly-aligned-luns-concept.html["Saiba mais sobre LUNs não alinhados"].
