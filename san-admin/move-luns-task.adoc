...
summary: É possível mover um LUN entre volumes dentro de uma storage virtual machine, mas não é possível mover um LUN entre SVMs. Os LUNs movidos entre volumes dentro de uma SVM são movidos imediatamente e sem perda de conectividade. 
---
= Mover um LUN
:allow-uri-read: 
:icons: font
