...
summary: Se um nó precisar ser desativado, você pode mover um SAN LIF para preservar suas informações de configuração, como o WWPN, e evitar o rezoneamento da malha de comutação. Como um SAN LIF precisa ser desativado antes de ser movido, o tráfego do host deve depender do software de multipathing do host para fornecer acesso ininterrupto ao LUN. Você pode mover SAN LIFs para qualquer nó em um cluster, mas não pode mover os SAN LIFs entre storage virtual machines. 
---
= Mova um SAN LIF
:allow-uri-read: 
:icons: font
