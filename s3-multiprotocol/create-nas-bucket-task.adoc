...
Por exemplo, você pode querer configurar uma política de bucket que conceda acesso somente leitura aos dados a um grande grupo de usuários e outra política de bucket que permita que um grupo limitado execute operações em um subconjunto desses dados.

A partir do ONTAP 9.18.1, você pode criar buckets NAS que permitem que aplicativos acessem dados em FlexCache volumes usando o protocolo S3. Todos os nós no cluster devem estar executando ONTAP 9.18.1 ou posterior. Antes de poder acessar um volume FlexCache usando o protocolo S3, você deve definir a  `-is-s3-enabled` opção para  `true` link:../flexcache/create-volume-task.html["sobre o FlexCache volume"]. O parâmetro é definido como  `false` por padrão.
...
.System Manager
--
Adicione um novo bucket S3 NAS em uma VM de armazenamento habilitada para NAS.
