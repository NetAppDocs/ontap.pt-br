...
.Sobre esta tarefa
Quando um servidor de armazenamento de objetos é criado, um usuário raiz com UID 0 é criado. Nenhuma chave de acesso ou chave secreta é gerada para esse usuário raiz. O administrador do ONTAP deve executar o comando  `object-store-server user regenerate-keys` para definir a chave de acesso e a chave secreta para esse usuário.
