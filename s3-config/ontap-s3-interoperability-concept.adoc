...
* FlexClone technology (volumes, arquivos e LUNs)
...
* Políticas de QoS aninhadas
+
link:../performance-admin/policy-group-assignment-restrictions.html["Restrições de atribuição de grupo de política de QoS"]
...
Recursos adicionais
 a| 
* A movimentação de volumes é suportada para volumes individuais que compõem um FlexGroup volume.
...
* Camada de nuvem do FabricPool (link:../s3-config/index.html["apenas S3 nativo"])
* Camada local do FabricPool (link:../s3-multiprotocol/index.html["Somente volumes NAS"])
* link:../flexcache/supported-unsupported-features-concept.html#supported-features["Volumes FlexCache"] (a partir do ONTAP 9.18.1)

 a| 
* A movimentação de volumes não é suportada para volumes FlexGroup que são usados por buckets do S3.
* FPolicy
* Qtrees
* Quotas
