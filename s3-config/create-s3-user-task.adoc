...
Para manter o acesso aos recursos S3 caso a chave de acesso original expire, a partir do ONTAP 9.19.1, você pode link:regenerate-access-keys.html#generate-a-second-s3-access-key["gerar uma segunda chave de acesso"] para cada usuário S3.
...
----
+
Você também pode definir manualmente sua própria chave de acesso e chave secreta ao criar um usuário nos  `-access-key` e  `-secret-key` parâmetros. Por exemplo:
...
.System Manager
--
. Selecione *Armazenamento > VMs de armazenamento*. Selecione a VM de armazenamento à qual você precisa adicionar um usuário, selecione *Configurações* e, em seguida, selecione image:icon_pencil.gif["Ícone de edição"] em S3.
