...
O ONTAP S3 suporta este cabeçalho adicional:
...
| PutBucketLifecycleConfiguration | ONTAP 9.13.1 * somente ações de expiração são suportadas 
...
A partir do ONTAP 9.19.1, ONTAP S3 oferece suporte à aplicação de gravações condicionais para uploads de objetos e exclusões condicionais para exclusões de objetos. As gravações e exclusões condicionais permitem evitar sobrescritas ou exclusões acidentais de objetos. As operações condicionais funcionam somente se a solicitação for feita usando o método de assinatura AWS_V4 em uma conexão TLS. Os seguintes cabeçalhos condicionais e operações de objeto são suportados:

|===
| Operação do objeto | Cabeçalho condicional 
...
O ONTAP S3 suporta este parâmetro de consulta URI adicional:
`uploadId`
 a| 
ONTAP 9,8
...
O ONTAP S3 suporta estes cabeçalhos adicionais:
...
| DeleteObjects o ONTAP S3 suporta este cabeçalho adicional: * `x-amz-bypass-governance-retention` | ONTAP 9.11,1 
...
O ONTAP S3 suporta estes parâmetros de consulta URI adicionais:
...
O ONTAP S3 suporta estes parâmetros URI adicionais:
...
* O ONTAP S3 adiciona suporte para uploads em partes, assinando solicitações usando `x-amz-content-sha256:
