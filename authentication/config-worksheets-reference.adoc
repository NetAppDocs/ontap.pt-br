...
Começando com ONTAP 9.15,1, você fornece esses valores com o `security dynamic-authorization modify` comando. Para obter mais informações sobre a configuração de autorização dinâmica, link:dynamic-authorization-overview.html["descrição geral da autorização dinâmica"]consulte .
