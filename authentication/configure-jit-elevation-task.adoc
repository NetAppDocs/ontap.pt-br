...
* A elevação de privilégios JIT só é suportada para usuários que utilizam autenticação por senha, nsswitch ou de domínio para fazer login. A autenticação multifator (MFA) não é suportada para elevação de privilégios JIT.
