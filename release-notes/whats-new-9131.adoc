...
| xref:../snaplock/snapshot-lock-concept.html#enable-snapshot-copy-locking-when-creating-a-volume[Bloqueio de cópias Snapshot à prova de violações] | Você pode usar o System Manager para bloquear uma cópia Snapshot em um volume que não seja da SnapLock, a fim de proteger contra ataques de ransomware. 
