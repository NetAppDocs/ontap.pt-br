...
| xref:../snaplock/snapshot-lock-concept.html#enable-snapshot-copy-locking-when-creating-a-volume[Bloqueio de snapshot à prova de violações] | Você pode usar o Gerenciador do sistema para bloquear um snapshot em um volume que não seja da SnapLock, a fim de proteger contra ataques de ransomware. 
