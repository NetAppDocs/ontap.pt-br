...
* Saiba mais sobre https://docs.netapp.com/us-en/ontap-afx/release-notes/whats-new-9191.html["novos e aprimorados recursos de software ONTAP para sistemas de armazenamento NetApp AFX"^].
* Saiba mais sobre https://docs.netapp.com/us-en/asa-r2/release-notes/whats-new-9191.html["novos e aprimorados recursos de software ONTAP para sistemas de armazenamento NetApp ASA r2"^].
...
== Destaques do ONTAP 9.19.1

ONTAP 9.19.1 oferece recursos novos e aprimorados nas áreas de segurança, escalabilidade e gerenciamento de storage para dar suporte à modernização da infraestrutura de dados. Esses aprimoramentos ajudam você a escalar, operar com mais eficiência e proteger os dados para o futuro. Para obter uma lista completa dos novos recursos e aprimoramentos, consulte xref:whats-new-9191.adoc[O que há de novo no ONTAP 9.19.1].

* xref:../performance-admin/guarantee-throughput-qos-task.html[Políticas de rajada de QoS configuráveis]
+
Políticas configuráveis de pico de QoS permitem aumentos de desempenho controlados e com tempo limitado acima dos limites de QoS, melhorando a capacidade de resposta da carga de trabalho durante eventos de pico sem sacrificar a equidade ou exigir provisionamento excessivo.

* xref:../nfs-admin/enable-nfs-over-tls-task.html[NFS sobre TLS 1.3]
+
Protege os dados NFS em trânsito com criptografia TLS 1.3 baseada em padrões, ajudando a fortalecer a proteção e simplificar a adoção de NFS criptografado em escala empresarial. 

* xref:../s3-config/regenerate-access-keys.html[Várias chaves de acesso S3 por usuário]
+
Adiciona suporte para múltiplas chaves de acesso S3 ativas por usuário, permitindo a rotação contínua de credenciais sem tempo de inatividade do aplicativo e ajudando você a fortalecer a segurança e a conformidade, mantendo o acesso ininterrupto às cargas de trabalho de objetos.

* xref:../snapmirror-active-sync/protect-nas-task.html[SnapMirror active sync para NAS]
+
Com suporte para NAS, SnapMirror active sync agora é uma solução unificada de continuidade dos negócios que oferece RPO zero com failover transparente. 

* xref:../san-config/configure-fc-nvme-hosts-ha-pairs-reference.html[FC conectado diretamente]
+
Apresenta suporte seguro para placas FC HBA, permitindo conectividade de host confiável e estabelecendo uma base para segurança FC em trânsito com suporte de hardware, preservando o desempenho e atendendo aos requisitos de conformidade corporativa. 

* xref:../san-admin/multipath-automatic-lif-failover-support.html[Failover LIF ativo-ativo unificado]
+
Os sistemas AFF oferecem multipathing ativo-ativo com failover automático de LIF, mantendo todos os caminhos SAN ativos e otimizados para que a E/S do host continue imediatamente durante o failover. 

* link:https://docs.netapp.com/us-en/ontap-fli/get-started/learn-about-data-migration.html["Suporte a iSCSI no FLI"^]
+
Adiciona suporte a iSCSI para Importação de LUN Externa (FLI), permitindo migrar dados de bloco de sistemas de storage de terceiros para ONTAP usando a conectividade iSCSI existente com interrupção mínima. 

* xref:../s3-config/create-modify-bucket-policy-task.html[Escritas e exclusões condicionais no S3]
+
Habilita o suporte para gravações e exclusões condicionais com a API S3, permitindo que os aplicativos apliquem verificações de consistência de dados durante atualizações e remoções de objetos, reduzindo o risco de sobrescritas ou exclusões acidentais em cargas de trabalho de objetos concorrentes e distribuídas.

* xref:../concepts/path-failover-concept.html#nas-path-failover[Failover combinado de LIF]
+
O failover automático de LIF é suportado nos sistemas AFF e ASA para LIFs de protocolo iSCSI em configurações multipath ativo-ativo e ativo-ativo-somente iSCSI, e para LIFs de protocolo NVMe/TCP em configurações multipath ativo-ativo. 

* xref:../networking/improve-tcp-wan-performance.html[Desempenho de WAN aprimorado]
+
O desempenho aprimorado da WAN com TCP RACK melhora a taxa de transferência e a resiliência em redes congestionadas, acelerando o acesso a arquivos remoto e os serviços de dados entre sites. 

* xref:../networking/configure-tls-offload.html[KTLS acelerado por hardware para criptografia mais rápida do cluster de backend]
+
Aprimora a criptografia do cluster de backend ao introduzir kTLS acelerado por hardware, descarregando o processamento de criptografia para SmartNICs compatíveis para oferecer maior desempenho e menor sobrecarga da CPU, mantendo a comunicação segura e sempre ativa em todo o tráfego do cluster e de alta disponibilidade. 

* xref:../anti-ransomware/use-cases-restrictions-concept.html#snapmirror[Detecção de ransomware com inteligência artificial para cargas de trabalho replicadas de forma síncrona]
+
Estende a Proteção Autônoma contra Ransomware (ARP/AI) para SnapMirror síncrono (SAN e NAS) e para ambientes SAN com SnapMirror active sync, preservando os sinais de detecção e recuperação em arquiteturas de RPO zero. 

* xref:../concept-svm-admin-dashboard-sm.html[Monitore e gerencie a SVM usando o novo painel do ONTAP System Manager]
+
Oferece uma interface gráfica amigável para o gerenciamento de SVMs, eliminando a necessidade de interface CLI complexa. 
...
== Destaques do ONTAP 9.18.1

O ONTAP 9.18.1 oferece recursos novos e aprimorados nas áreas de segurança, escalabilidade e gerenciamento de armazenamento para dar suporte à modernização da infraestrutura de dados.  Essas melhorias ajudam você a escalar, operar com mais eficiência e proteger os dados para o futuro.  Para obter uma lista completa dos novos recursos e melhorias, consultexref:whats-new-9181.adoc[Novidades no ONTAP 9.18.1] .

* xref:../networking/configure-backend-encryption.html[Criptografia de alta disponibilidade do cluster de backend]
+
Capacidade de usar mTLS para criptografar a rede do cluster de backend.

* xref:../networking/configure_network_security_using_federal_information_processing_standards_fips.html[Suporte PQC para dados em trânsito]
+
Capacidade de usar algoritmos pós-quânticos para dados em trânsito.

* xref:../networking/ipsec-prepare.html[Suporte a IPv6 IPSec com descarregamento de hardware]
+
Suporte para IPSec com descarregamento de hardware com endereço IPv6.

* xref:../anti-ransomware/use-cases-restrictions-concept.html#supported-configurations[Suporte ARP/AI para volumes FlexGroup]
+
Os volumes NetApp FlexGroup suportam ARP/AI, proporcionando detecção e proteção avançadas contra ransomware baseadas em aprendizado de máquina.

* xref:../san-admin/nvme-copy-offload.html[Descarregamento de cópia para NVMe]
+
Acelera grandes transferências de dados, descarregando as operações de cópia diretamente para o armazenamento, reduzindo a carga da CPU e da rede do host.

* xref:../s3-snapshots/list-restore-s3-snapshot.html[Restaurações de snapshots de buckets S3 orientadas a armazenamento]
+
Permite restaurar objetos corrompidos em um bucket do S3 ou restaurar o bucket inteiro sobre um bucket existente.
...
O ONTAP 9.17.1 oferece inovações em proteção autônoma contra ransomware com tecnologia de IA para SAN e adiciona suporte a host front-end NVMe com sincronização ativa SnapMirror e gerenciamento hierárquico de grupo de consistência.  Essas melhorias reforçam a liderança da NetApp em segurança de dados e resiliência cibernética, melhorando o desempenho, a escalabilidade, a eficiência e a continuidade dos negócios do ONTAP . Para obter uma lista completa de novos recursos e aprimoramentos, consulte xref:whats-new-9171.adoc[Novidades no ONTAP 9.17.1] .

* xref:../anti-ransomware/index.html[Suporte para SAN com proteção autônoma contra ransomware]
+
O ARP/AI oferece suporte a volumes SAN com detecção de anomalias baseada em criptografia, introduz novos comandos para estatísticas detalhadas de entropia e unifica as mensagens de proteção contra ransomware no System Manager, que antes se concentravam em NAS. Limiares de detecção configuráveis e uma retenção de snapshots mais determinística proporcionam maior flexibilidade para diversas cargas de trabalho.

* xref:../system-admin/use-hsts-task.html[Suporte HTTP Strict Transport Security (HSTS)]
+
O ONTAP 9.17.1 oferece suporte ao HTTP Strict Transport Security para serviços web, permitindo a aplicação de comunicação HTTPS segura entre o navegador do usuário e o ONTAP.

* xref:../networking/ipsec-prepare.html[Descarregamento de hardware IPsec com grupos de agregação de links]
+
O ONTAP 9.17.1 oferece suporte ao descarregamento de hardware IPsec para grupos de agregação de links, estendendo o suporte ao descarregamento de hardware introduzido no ONTAP 9.16.1.
...
+
A sincronização ativa do SnapMirror adiciona suporte para acesso NVMe para cargas de trabalho VMware com acesso a hosts NVMe/TCP e NVMe/FC para clusters ONTAP de dois nós.
...
+
O ONTAP 9.16.1 adiciona mais comandos à estrutura MAV para proteção adicional contra ameaças internas maliciosas. Esses aprimoramentos incluem muitos comandos de gerenciamento de Consistency Group (CG), VScan e Autonomous Ransomware Protection (ARP/AI), além de comandos de configuração de NVMe.

* xref:../anti-ransomware/index.html[Proteção autônoma contra ransomware com aprimoramentos de AI (ARP/AI)]
+
A Proteção Autônoma contra Ransomware foi aprimorada com novos recursos de IA, permitindo que ela detecte e responda a ataques de ransomware com 99% de precisão e recall. Como a IA é treinada em um conjunto de dados abrangente, não há mais um período de aprendizado para a ARP/AI em execução em FlexVol volumes, e a ARP/AI inicia em modo ativo imediatamente. A ARP/AI também introduz um recurso de atualização automática independente de uma atualização do ONTAP para garantir proteção e resiliência constantes contra as ameaças mais recentes.
...
+
A desalocação de espaço (também chamada de "hole punching" e "unmap") agora é suportada para namespaces NVMe.  A desalocação de espaço ajuda volumes com provisionamento fino e namespaces NVMe a recuperar espaço não utilizado quando os dados são excluídos pelo aplicativo host.  Isso melhora muito a eficiência geral do armazenamento, especialmente com sistemas de arquivos com alta rotatividade de dados.
...
+
A partir do ONTAP 9.15.1, você também tem a opção de fazer backup do próprio link simbólico em vez dos dados para os quais ele aponta. Isso pode proporcionar diversos benefícios, incluindo melhor desempenho dos seus aplicativos de backup. Você pode habilitar o recurso usando a ONTAP CLI ou a ONTAP API REST.
...
** xref:../anti-ransomware/support-arpai.html#multi-admin-verification-with-arp-ai-protected-volumes[Suporte à verificação de vários administradores]
+
...
+
Se os constituintes do FlexGroup ficarem desequilibrados, o FlexGroup pode ser reequilibrado e gerenciado de forma não disruptiva a partir da ONTAP CLI, ONTAP REST API e System Manager. Para desempenho ideal, os membros constituintes dentro de um FlexGroup devem ter sua capacidade utilizada distribuída uniformemente.
...
* Melhorias no System Manager
+
O System Manager baixa automaticamente atualizações de firmware para discos, prateleiras e processadores de serviço, além de fornecer novas integrações com o Active IQ Digital Advisor (também conhecido como Digital Advisor), NetApp Console e gerenciamento de certificados.  Essas melhorias simplificam a administração e mantêm a continuidade dos negócios.
