...
| Proteção autônoma contra ransomware (ARP/AI) | O ARP/AI é ativado automaticamente por padrão em todos os novos volumes após um período de carência de 12 horas em plataformas compatíveis. | ONTAP 9.18.1 
...
| Tempo de vida (TTL) para credenciais positivas em cache para ONTAP SVMs | A partir do ONTAP 9.17.1, o tempo de vida (TTL) para credenciais positivas em cache para SVMs do ONTAP foi reduzido de 24 horas (86,400,000 milissegundos) para 1 hora (3,600,000 milissegundos). | ONTAP 9.17.1 
...
| Nuvem da SnapMirror | SnapMirror cloud aumenta o número de buckets S3 suportados de 6 para 100. | ONTAP 9.19.1 


| Limite de controle de fluxo de conexão S3 | O limite de controle de fluxo para conexões ONTAP S3 foi reduzido de 2 MB para 256 KB para instâncias em nuvem que possuem <= 32 GB de memória. | ONTAP 9.19.1 


| Tempo limite de conexão ociosa S3 | O tempo limite de conexão ociosa do S3 foi reduzido de 20 minutos para 5 minutos. | ONTAP 9.19.1 
...
| Peer de cluster | Aumentamos o limite de peer de cluster suportados de 255 para 400, melhorando a escalabilidade para ambientes com um grande número de relações de peer de cluster. | ONTAP 9.14,1 
...
| Transferência simultânea do SnapMirror | O número máximo de transferências simultâneas assíncronas em nível de volume aumentou de 100 para 200.  As transferências do SnapMirror Cloud aumentaram de 32 para 100 em sistemas de ponta e de 6 para 20 em sistemas de baixo desempenho. | ONTAP 9,8 
