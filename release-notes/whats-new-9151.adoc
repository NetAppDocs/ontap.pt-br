...
xref:../smb-admin/windows-backup-symlinks.html[Aplicativos de backup do Windows e links simbólicos no estilo Unix em servidores]
 a| 
Quando um aplicativo de backup do Windows encontra um link simbólico (symlink) no estilo Unix, o link é seguido e os dados reais são retornados pelo ONTAP e copiados para o backup. A partir do ONTAP 9.15.1, você também tem a opção de fazer backup do próprio link simbólico em vez dos dados para os quais ele aponta. Isso pode proporcionar diversos benefícios, incluindo melhor desempenho dos seus aplicativos de backup. Você pode habilitar o recurso usando a CLI do ONTAP ou a API REST do ONTAP.
...
Os grupos de consistência que usam a proteção assíncrona do SnapMirror agora oferecem suporte a até 80 volumes no grupo de consistência.
 ...
 a| 
xref:../consistency-groups/configure-task.html[Suporte para nível de privilégio administrativo para operações da API REST do ONTAP e da CLI do ONTAP com grupos de consistência]
 a| 
As operações da CLI do ONTAP e da API REST do ONTAP para grupos de consistência agora são suportadas no nível de privilégio administrativo.
