...
Para atualizar para a versão mais recente do ONTAP, link:../upgrade/prepare.html["Prepare-se para atualizar o ONTAP"]consulte .
...
| Atualização | Descrição 
 ...
 a| 
xref:../flexcache-writeback/flexcache-writeback-enable-task.html[Suporte de escrita FlexCache]
 a| 
Quando o writeback é ativado no volume do cache, as solicitações de gravação são enviadas para o cache local em vez do volume de origem, proporcionando melhor desempenho para ambientes de computação de borda e caches com cargas de trabalho com gravação intensa.
