...
summary: Antes de reverter ONTAP, você deve verificar a integridade do cluster, a integridade do storage e a hora do sistema. Você também deve verificar se não há jobs em execução no cluster. 
---
...
** O quórum primário por anel deve ser o mesmo para todos os nós.
+
Note que cada anel pode ter um quórum primário diferente.
