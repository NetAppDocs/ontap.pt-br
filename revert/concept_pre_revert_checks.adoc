...
* link:arp-warning-clear-revert.html["Responda aos avisos da Autonomous ransomware Protection sobre atividades anormais"].




| ONTAP 9.19.1  a| 
* Se você tiver alguma SVM com configurações de multipath ativo-ativo, você deve link:delete-active-active-svms.html["excluir as SVMs"].
* Se você modificou as configurações padrão em qualquer grupo de políticas de QoS criado no ONTAP 9.19.1 ou posterior, você deve link:modify-qos-policy-groups.html["reinicie-os"].
* Se você atualizou os volumes para ONTAP 9.19.1 e adicionou uma segunda chave de acesso S3 a qualquer usuário, você deve link:second-s3-access-key.html["remova essas chaves"].




| ONTAP 9.18.1  a| 
* Se a capacitação automática tiver sido configurada para ARP/AI como parte de uma atualização do ONTAP 9.18.1, você precisará link:anti-ransomware-disable-auto-enable.html["desative-o."].




| ONTAP 9.17.1  a| 
* Se você habilitou o recurso ONTAP ARP/AI para SAN, link:anti-ransomware-disable-san.html["desative-o."].
