...
+
O `-version` A opção refere-se à versão do ONTAP para a qual você está revertendo. Apenas os dois primeiros valores da versão do ONTAP são necessários. Por exemplo, se você estiver revertendo da versão 9.14.1 para a 9.13.1, o valor correto do `-version` A opção é 9.13.
...
. Após ambos os nós terem sido revertidos, reative o HA para o cluster:
+
[source, cli]
----
...
NOTE: Reativar o HA do cluster pode fazer com que a  `storage failover auto-giveback` configuração volte a ser verdadeira.
...
----
. Verifique a configuração  `storage failover auto-giveback` em ambos os nós:
+
[source, cli]
----
...
----
+
Se for necessária uma configuração diferente para o seu ambiente, utilize o comando  `storage failover modify -node <nodename> -auto-giveback <true|false>` para modificá-la.
...
* link:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-KBs/Why_auto-giveback_enabled_after_re-enabling_storage_failover_setting["NetApp Base de conhecimento: por que o auto-giveback é ativado após reativar a configuração de storage failover"^]
