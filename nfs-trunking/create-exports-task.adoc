...
Requisitos de exportação do ONTAP:

* A política de exportação padrão do volume raiz da SVM deve incluir uma regra para permitir que todos os clientes tenham acesso irrestrito via NFS.
+
Saiba mais sobrelink:../nfs-config/open-export-policy-svm-root-volume-task.html["Acesso do cliente NFS na SVM do ONTAP."]

* Você deve adicionar a mesma regra à política padrão se tiver criado uma política de exportação personalizada.
+
Saiba mais sobrelink:../nfs-admin/default-export-policy-svms-concept.html["Políticas de exportação padrão para ONTAP NFS SVMs."]
