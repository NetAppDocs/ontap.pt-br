...
A partir do ONTAP 9.18.1, é possível configurar a comunicação criptografada entre nós de pares de alta disponibilidade (HA). Para obter mais informações, consultelink:ha-pair-encryption.html["Configure a criptografia para o tráfego ONTAP HA."] .
...
* *Monitoramento e prevenção de portas*: Em configurações de cluster sem switch com dois nós, o sistema evita portas que apresentem perda total de pacotes (perda de conectividade). No ONTAP 9.8 e anteriores, essa funcionalidade estava disponível apenas em configurações com switch.
