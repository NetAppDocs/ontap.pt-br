...
summary: Se o sistema não enviar a mensagem AutoSupport esperada e você estiver usando HTTP ou HTTPS, você poderá verificar várias configurações para resolver o problema. 
---
...
[role="lead"]
Se o sistema não enviar a mensagem AutoSupport esperada e você estiver usando HTTP ou HTTPS, ou o recurso Atualização automática não estiver funcionando, você poderá verificar várias configurações para resolver o problema.
...
.Sobre esta tarefa
Essas etapas são para casos em que você determinou que o AutoSupport pode gerar a mensagem, mas não pode entregar a mensagem por HTTP ou HTTPS.
...
. A partir do ONTAP 9.10,1, se você ativou o recurso Atualização automática, verifique se você tem conetividade HTTPS com os seguintes URLs adicionais:
+
