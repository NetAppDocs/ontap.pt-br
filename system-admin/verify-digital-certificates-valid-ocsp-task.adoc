...
summary: 'O Protocolo de Status de Certificado Online (OCSP) permite que aplicativos ONTAP que utilizam comunicações Transport Layer Security recebam o status do certificado digital quando o OCSP está habilitado. Você pode habilitar ou desabilitar a verificação do status do certificado OCSP para aplicativos específicos a qualquer momento. Por padrão, a verificação do status do certificado OCSP está desabilitada.' 
---
