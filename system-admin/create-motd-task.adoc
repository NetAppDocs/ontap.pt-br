...
summary: 'Você pode criar uma mensagem do dia (MOTD) para comunicar informações a usuários autenticados da CLI. A MOTD é exibida em uma sessão de console ou em uma sessão SSH após a autenticação do usuário, mas antes da exibição do prompt do clustershell.' 
---
