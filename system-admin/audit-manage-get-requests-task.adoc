...
summary: Embora as solicitações SET sejam registradas por padrão, as solicitações GET não são. No entanto, você pode usar o comando settings para controlar se as solicitações GET enviadas pelo ONTAP HTML, pela ONTAP CLI ou pelas ONTAP APIs serão registradas no arquivo. 
---
...
[role="lead"]
Embora as solicitações SET sejam registradas por padrão e não possam ser desativadas, as solicitações GET não são registradas por padrão. No entanto, você pode controlar se as solicitações GET enviadas do ONTAP HTML (`-httpget`), da ONTAP CLI (`-cliget`) ou das ONTAP APIs (`-ontapiget`) são registradas no arquivo.
