...
NOTE: Se você estiver executando esta etapa no primeiro nó de um par HA, deverá marcar apenas esse nó como inelegível. Não altere o status do parceiro HA. Se o nó parceiro for selecionado como o novo mestre, você precisa verificar se ele possui o valor epsilon antes de torná-lo inelegível. Isso é feito quando você repete esses passos para remover o nó parceiro.
