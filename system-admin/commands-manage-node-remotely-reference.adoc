...
Saiba mais sobre as  `help` opções de comando disponíveis para SP e BMC em link:online-help-at-sp-bmc-cli-task.html["Veja as informações de ajuda na CLI do ONTAP SP/BMC"].

Saiba mais sobre o CLI BMC correspondente do ONTAP em link:bmc-cli-commands-reference.html["Comandos CLI suportados para o controlador de gerenciamento de placa base ONTAP"].
