...
summary: Você pode gerenciar a mensagem do dia no nível do cluster ou da SVM. Por padrão, a mensagem do dia configurada para o cluster também é habilitada para todas as SVMs. Além disso, uma mensagem do dia no nível da SVM pode ser configurada para cada SVM. A mensagem do dia no nível do cluster pode ser habilitada ou desabilitada para cada SVM pelo administrador do cluster. 
---
