...
summary: O servidor de gerenciamento de cluster, também chamado de adminSVM, é uma implementação especializada de máquina virtual de armazenamento que apresenta o cluster como uma única entidade gerenciável. Além de servir como o domínio administrativo de mais alto nível, o servidor de gerenciamento de cluster possui recursos que não pertencem logicamente a uma SVM de dados. 
---
