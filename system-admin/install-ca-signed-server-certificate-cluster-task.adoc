...
summary: Para permitir que um servidor SSL autentique o cluster ou a máquina virtual de storage como um cliente SSL, você instala um certificado digital com o tipo cliente no cluster ou na SVM. Em seguida, você fornece o certificado client-ca ao administrador do servidor SSL para instalação no servidor. 
---
