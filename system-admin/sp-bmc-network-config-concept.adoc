...
summary: Você pode habilitar a configuração automática de rede em nível de cluster para o SP. Você também pode deixar a configuração automática de rede do SP desabilitada e gerenciar a configuração de rede do SP manualmente no nível do nó. Há algumas considerações a serem feitas em cada caso. 
---
