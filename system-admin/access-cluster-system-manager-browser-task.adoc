...
summary: Se preferir usar uma interface gráfica em vez da interface de linha de comando para acessar e gerenciar um cluster, você pode fazê-lo usando System Manager, que está incluído no ONTAP como um serviço web, está habilitado por padrão e pode ser acessado por meio de um navegador. 
---
