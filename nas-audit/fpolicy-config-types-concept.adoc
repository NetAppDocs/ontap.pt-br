...
summary: Existem dois tipos básicos de configuração do FPolicy. Uma configuração utiliza servidores FPolicy externos para processar e agir de acordo com as notificações. A outra configuração não utiliza servidores FPolicy externos. Em vez disso, utiliza o servidor FPolicy nativo e interno do ONTAP para bloqueio simples de arquivos com base em extensões. 
---
