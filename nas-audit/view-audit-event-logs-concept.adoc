...
+
Você pode visualizar e processar `XML` logs de eventos de auditoria em aplicativos de terceiros que suportam o `XML` formato de arquivo. Ferramentas de visualização XML podem ser usadas para visualizar os logs de auditoria, desde que você tenha o esquema XML e informações sobre as definições dos campos XML. Para obter mais informações sobre o esquema XML e as definições, consulte o https://library.netapp.com/ecm/ecm_get_file/ECMLP2875022["Referência de Esquema de Auditoria ONTAP"^].
